...
The \index{open ball}\emph{open ball} is denoted as $B(p,R)$ or $B(p,R)_{\mathcal X}$;
...
Analogously, the \index{closed ball}\emph{closed ball} is denoted as $\bar B[p,R]$ or $\bar B[p,R]_{\mathcal X}$ and
...
A map $f\:{\mathcal X}\to {\mathcal Y}$ is called \index{continuous map}\emph{continuous} if for any convergent sequence $x_n\to x_\infty$ in ${\mathcal X}$,
