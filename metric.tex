...
If we say plane or space we mean Eucledean plane or space.
However the plane (as well as the space) admits many other metrics, for example the the so called Manhattan metric from the following exercise.
...
Another example of a bizarre metric space let us mention \emph{discrete space} --- arbitrary nonempty set $\mathcal X$ with the metric defined as $|x\z-y|_{\mathcal X}=0$ if $x=y$ and $|x-y|_{\mathcal X}=1$ otherwise.
...
The open ball is denoted as 
\[B(p,R)\quad\text{or}\quad B(p,R)_{\mathcal X},\]
the second notation is used if we need to emphasize that the ball lies in the metric space $\mathcal X$.
Analogously, the closed ball is denoted as
\[\bar B[p,R]\quad\text{or}\quad \bar B[p,R]_{\mathcal X}.\]

\begin{thm}{Exercise}\label{ex:B2inB1}
Let $\mathcal X$ be a metric space.
\begin{enumerate}[(a)]
\item Show that  $B(p,2)\supset B(q,1)$ for any $p,q\in \mathcal X$.
...
A map $f\:{\mathcal X}\to {\mathcal Y}$ is called continuous if for any convergent sequence $x_n\to x_\infty$ in ${\mathcal X}$,
...
A set $\Omega \subset \mathcal{X}$ is called \emph{open}\index{open set} if the complement $\mathcal{X} \setminus \Omega$ is a closed set.
Equivalently, $\Omega \subset \mathcal{X}$ is open if for any $z\in \Omega$, 
