...
In this case we also use the shortcut notations $|x-y|$ or $|x-y|_{\mathcal X}$  for the {}\emph{distance $\Dist(x,y)$ from $x$ to $y$ in $\mathcal X$}.\index{10aaa@$\lvert x-y\rvert$, $\lvert x-y\rvert_{\spc{X}}$}
...
Another example: the \index{discrete space}\emph{discrete space} --- an arbitrary nonempty set $\mathcal X$ with the metric defined as $|x\z-y|_{\mathcal X}=0$ if $x=y$ and $|x-y|_{\mathcal X}=1$ otherwise.
...
\subsection*{Balls}
Given a point $p$ in a metric space ${\mathcal X}$ and a real number $R\ge 0$, the set of points $x$ on the distance less then $R$ (at most $R$) from $p$ is called the \index{open ball}\emph{open} (respectively \index{closed ball}\emph{closed}) \emph{ball} of radius $R$ with center $p$.
...
\index{10b@$B(p,R)_{\mathcal X}$, $\bar B[p,R]_{\mathcal X}$}
...
Show that if $\bar B[p,2]\subset \bar B[q,1]$ for some points $p,q\in \mathcal X$, then $\bar B[p,2]\z=\bar B[q,1]$.
\end{subthm}
...
\begin{thm}{Definition}
 Let ${\mathcal X}$ be a metric space.
A sequence of points $x_1, x_2, \ldots$ in ${\mathcal X}$ is called \index{convergence of points}\emph{convergent}
if there is 
...
$$|x-y|_{\mathcal X}<\delta\quad \text{ implies that }\quad |f(x)-f(y)|_{\mathcal Y}<\eps.$$
...
\begin{thm}{Definition}
A subset $C$ of a metric space $\mathcal{X}$ is called \index{closed set}\emph{closed} if whenever a sequence $(x_n)$ of points from $C$ converges in $\mathcal{X}$, we have that $\lim_{n\to\infty} x_n \in C$.
