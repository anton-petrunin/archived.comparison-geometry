...
\length \gamma
&= 
\sup \{|\gamma(t_0)-\gamma(t_1)|+\dots+|\gamma(t_{k-1})-\gamma(t_k)|\},
...
If a curve is defined on an open or semi-open interval, then its length is defined as the least upper bound for lengths of all its arcs.
\end{thm}


\begin{thm}{Exercise-Definition}\label{ex:integral-length-0}
Let $\gamma\:[a,b] \to\RR^3$ be a curve.
Suppose that the function $\phi : [c,d] \to [a,b]$ is surjective, continuous, and monotonic.
...
Suppose $\gamma\:[a,b]\to \RR^3$ is a parametrized space curve.
...
Let $\alpha\:[0,1]\to\RR^3$ be simple path.
...
Note that any rectifiable curve $\gamma$ can be parametrized by its arc length.
Indeed, for fixed a value $t_0$, 
\[s(t)=
\left\{
\begin{tabu}{rcr}
...
Since boundary is closed, any convex curve is either closed or open (see Section \ref{sec:proper-curves}).
...
Note that it is sufficient to show that for any polygon  $P$ inscribed in $\alpha$ there is a polygon $Q$ inscribed in $\beta$ with equal or larger perimeter.
...
Then 
\[\perim P\le \perim Q,\]
where $\perim P$ denoted perimeter of $P$.
\end{thm}
...
\section{Crofton's formulas}\label{sec:crofton}

For a function $f: \mathbb{S}^1 \to \mathbb{R}$, we will denote its average value as $\overline{f(\vec u)}$.
For a vector $\vec w$, and unit vector $\vec u$, we will denote by $\vec w_{\vec u}$ the orthogonal projection of $\vec w$ to the line in the direction $\vec u$;
that is,
...
\]
where the average is taken for all unit vectors $\vec u$.
\end{thm}
...
The exact value of $k$ can be found by integration\footnote{It is the average value of $|\cos|$.}, but we will find it another way.
...
Therefore $\length\gamma_{\vec u}=4$ for any $\vec u$.
Therefore 
...
It remains to consider nonrectifiable case;
...
for any plane curve $\gamma$.
Therefore, if $\gamma$ has infinite length, then the average of lengths of $\gamma_{\vec u}$ is infinte as well.
\qeds
...
As before, we denote by $\vec w_{\vec u}$ the projection of $\vec w$ to the line of $\vec u$.
...
Given two points $x,y$ in $\spc{X}$, denote by $d(x,y)$ the greatest lower bound of lengths of all paths connecting $x$ to $y$; if there is no such path, then $d(x,y)=\infty$.
...
that is, $|x-y|_A$ is the greatest lower bound of the lengths of paths in $A$ from $x$ to $y$.\index{10aaa@$\lvert x-y\rvert_A$}
