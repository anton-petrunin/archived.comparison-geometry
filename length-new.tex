...
If a curve is defined on a open or semi-open interval, then its length is defined as the exact upper bound for lengths of all its arcs.
\end{thm}
...
\item Construct a simple curve $\gamma\:[a,b]\to\RR^3$ such that $\gamma'(t)=0$ almost everywhere.
...
it is a fractal curve which we are about to construct.
...
By that reason, most of the time it is more convenient to use the original parametrization.
...
Try to show that the statement holds for arbitrary closed plane curve $\alpha$, assuming only that $K$ has nonempty interior.
\end{thm}
...
Show that the length of space curve is proportional to the average length of its projections (a) to all lines and (b) to planes.
...
Note that the function $d$ satisfies all the axioms of metric (see \ref{app:metric-spcaes}) except it might take infinite value.
...
If the equality holds for any pair, then the metric is called \emph{length metric} and the space is called \emph{length-metric space}.
...
\item\label{adex:crofton:hemisphere} Give another proof of hemisphere lemma using part (\ref{adex:crofton:crofton}). 
\end{enumerate}
