...
(In particular, the formula \ref{eq:length} does not hold for\nobreakspace$\gamma$ despite that both sides are defined.).
\end{subthm}
...
\parit{Comment.}
Another proof of \ref{lem:perimeter} can be obtained using closest point projection; see Section\nobreakspace\ref{sec:closest-point-projection}.  
...
By Proposition\nobreakspace\ref{prop:convex-curve}, the length of the curve cannot exceed $8\cdot C$ --- the perimeter of the square. 
Hence the result.
\qeds
...
for any $t \in [a,b]$. Then, according to Exercise\nobreakspace\ref{ex:integral-length},
\begin{align*}
\length\gamma
&=\int_a^b|\gamma'(t)|\cdot dt=
\\
...
Use this statement to give another solution to Exercise\nobreakspace\ref{ex:convex-hull}.
\end{thm}
...
Without loss of generality we may assume that $r$ lies on\nobreakspace$\gamma_1$. 
