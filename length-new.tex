...
If a curve parameterized by an open or semi-open interval, then its length is defined as the least upper bound of the lengths of all its restrictions to closed intervals.
\end{thm}
...
Then the polygonal line $p_0\dots p_k$ is said to be an \index{inscribed polygonal line}\emph{inscribed} in $\gamma$.
...
\begin{subthm}{adex:integral-length:b} Construct a non-constant curve $\gamma\:[a,b]\to\RR^3$ such that $\gamma'(t)=0$ almost everywhere.
...
\]
where $\gamma_{ \vec u}$ is the curve defined as $\gamma_{\vec u}(t) := (\gamma (t))_{\vec u}$.
\end{thm}
...
Note that for a smooth plane curve $\gamma\:[a,b]\to\RR^2$,
\[  \gamma_{\vec u}^{\prime}(t) = (\gamma ^{\prime} (t))_{\vec u} \text{ and }     | \gamma^{\prime}_{\vec u} (t) |  =  | \langle \vec u , \gamma^{\prime} (t) \rangle |     \]
...
If the equality holds for all pairs, then the metric $|{*}-{*}|$ is said to be a \index{length metric}\emph{length metric} and the corresponding metric space is called a \index{length-metric space}\emph{length-metric space}.
