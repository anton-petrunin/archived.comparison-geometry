...
Show that
\begin{enumerate}[(a)]
\item $\length \alpha\ge \int_a^b|\alpha'(t)|\cdot dt$,\footnote{Hint: apply the fundamental theorem of calculus for each segment in a given partition.}
\item $\length \alpha\le \int_a^b|\alpha'(t)|\cdot dt$.\footnote{Consider a partition such that $\alpha'$ is nearly constant on each of its segments.}
\end{enumerate}
Conclude that 
