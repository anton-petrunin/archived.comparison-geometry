...
\end{subthm}
\begin{subthm}{}
Conculude that for any $p\in \T_p$,
the exponential map $\exp_p\:\T_p\z\to\Sigma$ is a diffeomorphism.
...
Note that if $\gamma$ has a self-intersection, then it contains a simple loop;
...
Since $0<\phi_s<\pi$, we have that the self-intersection at the base of the loop of $\gamma_s$ is transverse; therefore it survives in $\gamma_t$ for all $t$ sufficiently close to $s$.
