...
Note that if $\Sigma$ is a plane, then the statement of theorem follows from \ref{prop:total-signed-curvature}.
...
Fix $p\in \partial\Delta$ consider the parallel translation $\iota\:\T_p\to\T_p$ along $\partial\Delta$.
According to \ref{prop:pt+tgc}, $\iota$ is a clockwise rotation by angle $\tgc{\partial\Delta}_\Sigma$.

According to \ref{lem:spherical-image}, $\iota$ is also parallel translation $\partial[\nu(\Delta)]$ in $\SS^2$.
In particular $\iota$ is a clockwise rotation by angle $\tgc{\partial[\nu(\Delta)]}_\Sigma$.
By \ref{prop:area-of-spher-polygon} 
\[\tgc{\partial[\nu(\Delta)]}_{\SS^2}+\area[\nu(\Delta)]=2\cdot \pi.\]
Therefore 
$\iota$ is a counterclockwise rotation by angle $\area[\nu(\Delta)]$
...
This is so for plane disc, see \ref{eq:sum=2pin}.
One can think that $\Delta$ is a result of a continuous deformation of plane disc.
The integer $n$ can not change in the process of deformation since the left hand side in \ref{eq:sum=2pin} is continuous along the deformation.
