...
Suppose that the contour $\partial\Delta$ is oriented such that $\Delta$ lies on its left. 
Then its external angles are  $\pi-\alpha$, $\pi-\beta$ and $\pi-\gamma$.
Therefore 
\[\tgc{\partial\Delta}=3\cdot\pi-\alpha-\beta-\gamma\]
which implies that
\[\GB(\Delta)=0.\]
 
Note that we can subdivide a given spherical polygon $P$ inто triangles by dividing a polgon in two on each step.
...
Suppose $\alpha\:[0,1]\to \Sigma$ is a smooth regular loop such that $\alpha'(0)+\alpha'(1)=0$.
