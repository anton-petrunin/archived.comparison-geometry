...
\end{cases}
\]
Motreover, they have the same initial values at $s_0$.
...
If $\gamma$ is a piecewise smooth and regular plane curve, then we define its total signed curvature as the sum of the total signed curvatures of its arcs plus the sum of the signed external angles at its joints;
...
By \ref{eq:tsc-tc},  the value in \ref{eq:Psi-alpha-beta} vanishes, or equivalently
...
denote by $\skur(s)$ the signed curvature of $\gamma$.

For fixed $s$, the unit-speed curve $\sigma$ of constant signed curvature $\skur(s)$ that starts at $\gamma(s)$ in the direction $\gamma'(s)$ is called the \emph{osculating circline} of $\gamma$ at~$s$.
\end{thm}
...
The center and radius of the osculating circle at a given point are called \emph{center of curvature} and \emph{radius of curvature}.
...
and they share the tangent line at those time parameters;
...
if they point in opposite directions, we say that the curves are \emph{controriented} at the  time parameters $t_1$ and $t_2$.
...
Then there is a line that supports $\gamma$ at two points, say $x$ and $y$. The points $x$ and $y$ divide $\gamma$ in two arcs $\gamma_1$ and $\gamma_2$, both distinct from the line segment $[x,y]$.
Note that one of the arcs is parametrized from $x$ to $y$ and the other from $y$ to $x$.
Passing to a smaller arc if necessary we can ensure that only its endpoints lie on the line; 
so we can apply the lens lemma from which we get that the arcs $\gamma_1$ and $\gamma_2$ contain points with signed curvatures of opposite signs.
\qeds
...
otherwise one could increase its radius slightly while keeping the circle in $F$.
The circle $\sigma$ will be called \emph{incircle} at $p$.
...
Repeat this construction recursively.
...
We present a proof based on the key lemma in the previous section.
In fact we prove the following stronger version of the four-vertex theorem:
...
\parit{Proof of \ref{thm:4-vert} and \ref{thm:4-vert-supporting}.}
...
We showed that \ref{thm:4-vert-supporting} implies the four-vertex theorem (\ref{thm:4-vert});
it remains to prove \ref{thm:4-vert-supporting}.
...
We showed existence of two osculating circles that support $\gamma$ from inside;
it remains to show existence of two osculating circles that support $\gamma$ from outside.

If one applies to $\gamma$ an inverse with the center inside $\gamma$, then the obtained curve $\gamma_1$ also has  two osculating circles that support $\gamma_1$ from inside.
...
(Alternatively, in order to get the osculating circles supporting $\gamma$ from outside, one can repeat the proof of key lemma taking instead of incircle the circline of maximal signed curvature that supports the curve from outside, assuming that $\gamma$ is oriented so that the region on the left from it is bounded.)
\qeds
