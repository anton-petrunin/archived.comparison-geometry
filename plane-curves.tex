...
Since $\nu'(s)=-\skur(s)\cdot\tau(s)$, the directiion of $z'(s)$ can not have constant direction on a nentrivial interval;
...
\section*{Tangent and supporting circlines}

Suppose $\gamma$ is a smooth regular plane curve.
We say that circline $\sigma$ is tangent to $\gamma$ at $t_0$ if
...
tha is, the tangent vectors $\gamma'(t_0)$ and $\sigma'(t_1)$ point in the same direction.
If not we can reverse the parametrization of $\sigma$.
If both curves are given with arc length parametrization, then it simply means that $\gamma'(t_0)=\sigma'(t_1)$.
...
Therefore we can assume that $\sigma$ is cooriented with $\gamma$  at $t_0$ and we say that $\sigma$ supports $\gamma$ from the left (right) if $\gamma$ lies on the right (correspondingly left) side from $\sigma$. 
...
The circle $\sigma$ on the diagram locally supports curve $\gamma$ on the left at $p$, but does not support it globally --- since $\gamma$ crosses $\sigma$ at a latter time.
...
Assume $\sigma$ is a circline that that locally supports $\gamma$ at $t_0$ from the rigth (correspondingly left). 
...
Assume a closed smooth regular plane curve $\gamma$ runs between parallel lines on distance 1 from each other.
Show that there is a point on $\gamma$ with curvature at least $1$.
\end{thm}

\begin{thm}{Exercise}
Assume a closed smooth regular plane curve $\gamma$ runs inside of a triangle $\triangle$ with inradius 1; that is, the  inscribed circle or $\triangle$ has radius 1.
Show that there is a point on $\gamma$ with curvature at least~$1$.
\end{thm}
...
Moreover $\gamma'(t_0)$ points in the direction of $q$;
that is, if we go along $\sigma$ in the direction  of $\gamma'(t_0)$ then we have to start at $p$ and end at $q$.
If the direction is opposite, then the arc of $\gamma$ from $s$ to $y$ would be trapped in the curvelinear triangle $xsp$ bounded by arcs of $\sigma$, $\gamma$ and the line segment $[px]$.
...
\[\kur(t_0)\ge \kur,\]
where $\kur(t_0)$ is signed curvature of $\gamma$ at $t_0$ and $\kur$ is the curvature of $\sigma$.
Evidently $\kur>0$, hence the result.
\qeds
...
To prove the theorem it is sufficient to note since curvature is bounded by 1, every osculating circle has radius at least 1 and then it remains to apply \ref{thm:moon}.

This theorem gives a simple but nontrivial example of the so called ``local to global theorems'' --- based on some local data (in this case the curvature of a curve) we conclude a global property (in this case existence of a large disc surrounded by the curve).
For convex curves, this result was known much earlier \cite[\S 24]{blaschke}.

%???+PIC

A straightforward approach to the latter theorem would be to start with some disc in the region bounded by curve and blow it up to maximize its radius.
...
Recall that if an osculating circle supports the curve at the same point $p$, then $p$ is a vertex (see \ref{ex:vertex-support}).
...
but it is not as beautiful as the one given below. %??? but it does not stand the proof below
...
Note that it has to touch $\gamma$ at another point;
...
therefore they they coincide.
But by construction $p_2\in \sigma_2$ and $p_2\notin \sigma_1$ --- a contradiction.
...
By \ref{eq:length<length/2}, we get that 
...
Now suppose that there is the only one point $p\in\gamma$ such that corresponding circle is osculating.
