...
\[\Psi(\gamma)
=
\int_\mathbb{I} \skur(s)\cdot ds,\eqlbl{eq:tsc-k}\]\index{10psi@$\Psi(\gamma)$}
...
if these directions are opposite, they are called \index{controriented!curves}\emph{controriented}.
...
Suppose $\gamma_1$ is simple proper curve, so it divides the plane into two closed region that lie on left and right from $\gamma_1$. Then we say that $\gamma_1$ \index{global support}\emph{globally supports} $\gamma_2$ at $t_2$ 
...
We say that a point lies \index{inside}\emph{inside} (respectively, \index{outside}\emph{outside}) of $\gamma_2$ if it lies in the bounded region (respectively, unbounded) region.
In this case we say if $\gamma_1$ supports $\gamma_2$ \index{support from inside}\emph{from inside} (\index{support from outside}\emph{from outside}) if $\gamma_1$ supports $\gamma_2$ and lies in the region inside it (respectively outside it). 

Note that if $\gamma_1$ and $\gamma_2$ share a point $p=\gamma_1(t_1)=\gamma_2(t_2)$ and not tangent at $t_1$ and $t_2$, then at time $t_2$ the curve $\gamma_2$ crosses $\gamma_1$  moving from one of its sides to the other.
...
If $\gamma$ is simple we could say that the point $p=\gamma(s)$ is a vertex of $\gamma$.
...
Note that according to \ref{thm:DNA} for any closed smooth regular curve that runs in a unit disc, the average of its absolute curvature at least 1; in particular it has a point with absolute curvature is at lest 1.
...
The three exercises above are a baby cases of a \ref{ex:moon-rad}; try to find a direct solution, without using \ref{thm:moon}.
...
The same argument shows that the curvature at $w$ is nonnegative, which is weaker than the required positive curvature.
...
Note that the curvature $\skur_\sigma$ of $\sigma$ has to be larger than $\skur(p)_\gamma$.
Indeed, by \ref{prop:supporting-circline} we have $\skur_\sigma\ge \skur(p)_\gamma$, and, in the case of equality, $\sigma$ is the osculating circle at $p$.
The latter is impossible by the assumption.

It follows that $\sigma$ has to touch $\gamma$ at another point.
Otherwise we can increase $\sigma$ slightly keeping it inside $F$.

Indeed, since $\skur_\sigma> \skur(p)_\gamma$, 
by \ref{prop:supporting-circline} we can choose a neighborhood $U$ of $p$ such that after any sufficiently small increase of $\sigma$, the intersection $U\cap \sigma$ is still in $F$.
On the other hand, if $\sigma$ does not touch $\gamma$ at another point, then after any sufficiently small increase of $\sigma$ the complement $\sigma\backslash U$ is still in $F$.
...
It known for many different proofs and generalizations.
