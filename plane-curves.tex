...
defined in Section\nobreakspace\ref{sec:frenet-frame};
...
We may use notation $\skur(s)_\gamma$ if we need to specify the curve\nobreakspace$\gamma$.

Note that 
\[\kur(s)=|\skur(s)|;\]
that is, up to sign, the signed curvature $\skur(s)$ equals the curvature $\kur(s)$  of $\gamma$ at $s$ defined in Section\nobreakspace\ref{sec:curvature};
...
Note that by the fundamental theorem of calculus, we have $\theta'(s)\z=\skur(s)$ for all\nobreakspace$s$.
...
the curve $\gamma_\ell(t)\z=\gamma(t)+\ell\cdot\norm(t)$; it is called a {}\emph{parallel curve of $\gamma$ at signed distance\nobreakspace$\ell$}.
...
Describe an example showing that formula \ref{eq:length(parallel-curve)} does not hold for all\nobreakspace$\ell$. 
\end{subthm}
...
The unit-speed curve $\sigma$ of constant signed curvature $\skur(s)$ that starts at $\gamma(s)$ in the direction $\gamma'(s)$ is called the \index{osculating!circline}\emph{osculating circline} of $\gamma$ at\nobreakspace$s$.
