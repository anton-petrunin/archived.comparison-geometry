...
Suppose $\gamma_1$ is simple proper curve, so it divides the plane into two closed regions that lie on left and right from $\gamma_1$. Then we say that $\gamma_1$ \index{global support}\emph{globally supports} $\gamma_2$ at $t_2$ 
...
If not, then the arc of $\gamma$ from $w$ to $y$ would be trapped in the curvelinear triangle $xwp$ bounded by the line segment $[p,x]$ and the arcs of $\sigma$, $\gamma$.
