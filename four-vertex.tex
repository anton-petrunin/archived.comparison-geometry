...
%\begin{thm}{Generalized lemma}
%Let $\gamma$ be a circle of curvature $\kappa$.
%Suppose that two points $x,y\in\gamma$ and a simple curve $\alpha$ on the left side from $\gamma$ from $x$ to $y$.

%Assume there is an other circle $\gamma'$ of the same curvature $\kappa$ that contains $\alpha$ on the right side 



%Suppose $L$ is a lens bounded by two arcs $\phi$ and $\psi$ with signed curvature $\kappa>0$ and both arcs run from $p$ to $q$.
%Assume that $L$ lies on right from $\phi$ and on left from $\psi$ and the points $p,x,y,q$ lie in the same order on $\gamma$.
%If a curve simple smooth curve $\alpha$ runs inside $L$ from $x$ to $y$, then it has a point with curvature at most $\kappa$.
%\end{thm}
