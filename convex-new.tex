...
Note that the region $R$ is the closure of ints interior, therefore $R$ is convex as well.
...
that is there is a positive function $\rho\:\SS^2\to\RR$ such that $\Sigma$ is formed by points $\rho(\xi)\cdot \xi$ for $\xi\in \SS^2$.

Let us show that $\rho$ is a smooth function.
Fix a point $p=(x_p,y_p,z_p)$ on $\Sigma$.
Consider a local implicit description of $\Sigma$ at $p$ as a solution of equation $f(x,y,z)=0$ with nonvanishing gradient;
so $f(p)=0$.
Note that for any point $q$ in a neighborhood of $p$, we have that 
\[f(q)=0 \iff q\in \Sigma \iff q=\rho(\xi)\cdot\xi\] for some $\xi\in \SS^2$.
In other words $f$ defines implicitly $\rho$ as in the implicit function theorem.
...
Note that the function $f\circ S$ is smooth and
$\tfrac{\partial f\circ S}{\partial \rho}(p)=D_\eta f(p)\ne 0$.
Applying implicit function theorem, we get that $\rho$ is smooth in a neighborhood of $\eta$;
since $\eta$ is arbitrary it is smooth on whole $\SS^2$.
\qeds
...
That is, $\Sigma$ is a graph of a smooth convex function $z=f(x,y)$.
...
By inverse function theorem the $\Omega$ is open.\qeds
...
Show that any open surface $\Sigma$ with positive Gauss curvature
