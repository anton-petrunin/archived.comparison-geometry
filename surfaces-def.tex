...
For example, any embedding $s\:\SS^2\to\RR^3$ might be called a \index{topological sphere}\emph{topological sphere}
and if $s$ is smooth and regular, then it might be called \index{smooth sphere}\emph{smooth sphere}.
(A smooth regular map $s\:\SS^2\to\RR^3$ which is not necessary an embedding is called a \index{smooth immersion}\emph{smooth immersion}, so we can say that $s$ describes a \index{smooth immersed sphere}\emph{smooth immersed sphere}.) 
...
Th map $L_p s$ is called a \index{linearization}\emph{linearization} (or \index{differential}\emph{differential}) of $s$ at $p$.
...
The map $\Norm\:\Sigma\to \SS^2$ defined by $p\mapsto \Norm_p$ is called \index{spherical map}\emph{spherical map} or \index{Gauss map}\emph{Gauss map}.

For surfaces, the spherical map plays essentially the same role as the tangent indicatrix for curves.

\section{Sections}%???redo into sections
