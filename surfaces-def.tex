...
Its proof is not at all trivial; a standard proof uses the so-called \emph{Alexander's duality} which is a classical technique in algebraic topology. %???+ref
...
Let $\phi\:\Sigma_1\to\Sigma_2$ be another smooth map between smooth surfaces $\Sigma_1$ and $\Sigma_2$.
Suppose that $q=\theta(p)\in\Sigma_1$;
observe that $L_p(\phi\circ\theta)=L_q\phi \circ L_p\theta$,
It follows that
\[\jac_p(\phi\circ\theta)
=
\jac_q\phi\cdot\jac_p\theta,\eqlbl{eq:jac-composition}\]
where $q=\theta(p)\in\Sigma_1$.
