...
Its proof is not at all trivial; a standard proof uses the so-called \index{Alexander's duality}\emph{Alexander's duality} which is a classical technique in algebraic topology \cite[see][]{hatcher}.
...
If $s\:U\to \RR^3$ is a smooth regular embedding %???add embedding to prelim???
...
and denoted by
\[D_{\vec{w}}f,\quad D_{\vec{w}}f(p), \quad\text{or}\quad D_{\vec{w}}f(p)_\Sigma\] \index{$D_{\vec{w}}f$}
...
does not depend on the choice orthonormal basises in $\T_p$ and $\T_{ s(p)}\Sigma_1$. \label{page:|L|}\index{$d_p s$}
...
it is uniquely defined up to sign.\index{$\Norm$}
