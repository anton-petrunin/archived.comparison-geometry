...
Despite it might look obvious, its proof is not trivial at all; a standard proof uses the so-called \index{Alexander's duality}\emph{Alexander's duality} which is a classical technique in algebraic topology \cite[see][]{hatcher}.
...
The map $\Norm\:\Sigma\to \mathbb{S}^2$ defined by $p\mapsto \Norm_p$ is called the \index{spherical map}\emph{spherical map} or \index{Gauss map}\emph{Gauss map}.
