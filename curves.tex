...
The proof of this statement is an easy exercise in general topology. % a formal proof can be found in???.
...
A \emph{parametrized curve} is defined as a continuous map $\gamma : G \to X $ from a circle or a real interval (open, closed or semi-open) to a metric space $X$. 
For a parameterized curve we do not assume that $\gamma$ is injective; so it might have \emph{self-intersections}.
...
Consider the set $\gamma \subset \mathbb{R}^2$ of solutions of the equation $f(x,y)=0$.
...
In particular, any implicitly defined plane or space curve is proper.%??? ref (it is really easy)
