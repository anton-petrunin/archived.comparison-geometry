...
Recall that \index{real interval}\emph{real interval} is defined as a connected subset of the real numbers.
...
If $\GG$ is an open interval or a circle, we say that $\gamma$ is a \index{endpoints}\emph{curve without endpoints}, otherwise it is 
...
When $\GG$ is a closed interval, $\gamma$ is called a \index{simple arc}\emph{simple arc}.
...
A \index{parametrized curve}\emph{parametrized curve} is defined as a continuous map $\gamma$ from a circle or a real interval (open, closed or semi-open) to a metric space. 
...
Curves in Euclidean space or plane are called \index{space curve}\emph{space curves} or, respectively, \index{plane curve}\emph{plane curves}.
...
A simple space curve is called \index{smooth curve}\emph{smooth} (respectively, \index{regular curve}\emph{regular}) if it admits a smooth (respectively, regular) parametrization.
...
Any smooth regular closed curve can be described by a \index{periodic parameterization}\emph{periodic} parametrized curve $\gamma\: \RR\to \mathcal{X}$; that is, a curve such that $\gamma(t+\ell)=\gamma(t)$ for a fixed period $\ell > 0$ and all $t$.
