...
Assume that $\gamma$ has unit-speed.
...
In order to prove \ref{|sigma'|<kur-theta'}, note that there is an orthonormal frame $\tan(s), \vec v(s), \vec w(s)$ 
such that $\tan=\gamma'$ and $\sigma=\cos\theta\cdot\tan+\sin\theta\cdot\vec v$; in particular $\sigma$ lies in the plane spanned by $\tan$ and $\vec v$.
Observe that $\tan'\perp \tan$ and $\langle \tan,\sigma\rangle=\cos\theta$;
therefore
\begin{align*}
\kappa&=|\tan'|\ge
\\
&\ge  -\langle \tan',\vec v\rangle=
\\
&= -\frac{\langle \tan',\sigma\rangle}{\sin\theta}=
\\
&= \frac{\langle \tan,\sigma'\rangle-\langle \tan,\sigma\rangle'}{\sin\theta}=
\\
&= |\sigma'|+\theta'.
\end{align*}

There is \parit{an alternative solution} by means of elementary geometry based on \ref{prop:inscribed-total-curvature}.
