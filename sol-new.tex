...
\parbf{\ref{ex:convex-polyhon+self-intersections};} \ref{SHORT.ex:convex-polyhon}.
Let us consider $\Delta$ with induced length-metric.

Note that repeating the proof of \ref{thm:comp:toponogov} we can show that the the comparison holds in $\Delta$.

Now choose a vertex of $\Delta$ and connect it to the remaining vertexes by minimizing geodesics.
It divides $\Delta$ into triangles, take a model triangle so that they share sides as in $\Delta$.
...
By \ref{ex:convex-polyhon}, we can find a pentagon on the plane with convex sides of the same length and angles at most as big.

It remains to show that such plane pentagon does not exist.
