...
<<<<<<< HEAD
Then there is a point $p\in \Omega$ such that for any natural $n$ there is a point $q_n\in Q$ with $|p-q_n|_{\spc{X}}\z<\tfrac 1n$; in particular $q_n\to p$ and $n\to \infty$.
...
>>>>>>> e2e11134d7bacd0432676a980f98d23777565d10

\parbf{\ref{ex:9}.} The image of $\gamma$ might have the shape of the digit $8$ or $9$.
...
$\tau(t_1)=\tau(t_2)$ if and only if there is a connected component $(a,b)$ of $\hat \Omega$ such that $t_1,t_2\z\in [a,b]$.
This construction is similar to the construction of  the devil's staircase.
The simple arc we are looking for is $\beta \circ \sigma $, where $\sigma: [0,1] \to [0,1]$ is any right inverse of $\tau$.

=======
Define $\beta(t)=\alpha(a)$ for any $t$ in a connected component $(a,b)\subset\hat \Omega$.
Note that for any $x\in [0,1]$ the set $\beta^{-1}\{\beta(x)\}$ is connected.
...
Note that the first equation describes the unit sphere centered at the origin
<<<<<<< HEAD
and the second equation describes a cylinder over the circle in the $(x,y)$-plane with diameter with center $(-\ell / 2 , 0)$ and radius $\ell / 2$.
=======
and the second equation describes a cylinder over the circle in the $(x,y)$-plane with opposite points $(0,0)$ and $(0,\ell)$.
>>>>>>> e2e11134d7bacd0432676a980f98d23777565d10
...
=======
The case $\ell=1$ shown on the diagram.
>>>>>>> e2e11134d7bacd0432676a980f98d23777565d10
...
<<<<<<< HEAD
Show that inversion of the plane $(x,y)\mapsto (\tfrac{x}{x^2+y^2},\tfrac{y}{x^2+y^2})$ maps our curve to a curve whose union with the origin is a closed curve.
Apply Jordan's theorem for the obtained curve and use the inversion again.

\parbf{\ref{ex:integral-length-0}.} Observe that $c=\tau_0<\dots<\tau_n=d$ is a partition of $[c,d]$ if and only if $t_i=\phi(\tau_i)$ is a partition of $[a,b]$ and apply the definition of length (\ref{def:length}).
=======
Show that inversion of the plane $(x,y)\mapsto (\tfrac{x}{x^2+y^2},\tfrac{y}{x^2+y^2})$ maps our curve to a closed curve with removed origin.
Apply Jordan's theorem for the obtained curve and use the inversion again.
...
Observe that $a=t_0\le t_1\le\z\dots\le t_k=b$
if there is a sequence $c=\tau_0\le \dots<\tau_k=d$ such that $t_i=\phi(\tau_i)$.
Make a conclusion.
>>>>>>> e2e11134d7bacd0432676a980f98d23777565d10
...
=======
Observe that $\gamma$ can be divided into 4 arcs each of which is a scaled copy of $\gamma$ with factor $\tfrac13$.
...
Note that $\gamma_{\vec u}$ runs back and forth along $\ell_{\vec u}$, we get 
...
It remains to show that the sequence of curves defined by
...
<<<<<<< HEAD
Note that without loss of generality we can assume the curve in \ref{thm:DNA} is described by a loop $\gamma\:[0,\ell]\to\RR^3$ parameterized by arc-length.
=======
Note that without loss of generality we can assume the curve in \ref{thm:DNA} is described by a loop $\gamma\:[0,\ell]\to\RR^3$ parametrized by its arc-length.
>>>>>>> e2e11134d7bacd0432676a980f98d23777565d10
...
choose $p_{2\cdot i}$ and $p_{2\cdot i+1}$ sufficiency close to $\gamma(s_i)$.
Observe that we can assume the direction of $p_{2\cdot i+1}-p_{2\cdot i}$ is sufficiency close to $\vec u_i$ for each $i$.
Conclude that \ref{eq:tc=<tc} holds true for the polygonal line $\beta=p_0\dots p_{2\cdot k}$.
...
<\pi$, or, equivalently, 
\[\tc{xyzx}>\pi;\]
here we take $xyzx$ as a polygonal line which is not closed. 
It remains to apply \ref{prop:inscribed-total-curvature}.

\parbf{\ref{ex:quadrisecant}.}
Observe that 
\[\tc{acbd}=4\cdot\pi,\]
where $acbd$ denotes the closed polygonal line.
It remains to apply \ref{prop:inscribed-total-curvature}.
...
Let $(u,v)\mapsto s(u,v)$ be a small cart of $\Sigma$ such that $p=s(0,0)$.
Denote by $\vec k$ the unit vector in the direction of $z$ axis.

Show that we can assume that $\langle s(u,v),\vec k\rangle$ has definitive sign in a punctured neighborhood of $0$.
...
Apply \ref{prop:S=-D} and together with the following identities:
...
We can assume that $\gamma$ is parametrized by its arc-length.
...
Whence $f_t$ is regular for $t\approx 0$.
...
Apply \ref{obs:k1-k2} and the definition o mean curvature.
...
Denote by $L$ the cut locus of $V$.
...
Show that its cut locus $L$ is formed by a few smooth surfaces that meets by three at their boundary points.
Show that $L$ is not simply connected; that is, there is a loop in $L$ that cannot be deformed continuously to a trivial loop.
...
\parbf{\ref{ex:saddle-convex}.} Suppose $p$ is a point of local maximum.
...
Observe that $\Gamma_p$ has only two ways to escape to infinity.
...
%\parit{\ref{SHORT.ex:unique-geod:diffeomorphism}.} ???PROBLEM --- we need second fed of inj rad.
...
\parbf{\ref{ex:dist-to-bry}.} %???generalize
