...
if there is a sequence $c=\tau_0\le \dots\le\tau_k=d$ such that $t_i=\phi(\tau_i)$.
Make a conclusion.
...
For a unit-speed parameterization $\gamma$, the velocity $\gamma^{\prime}$ has to be constant, hence $\gamma $ is an affine function $\gamma(t) = p+tq$ for some $p,q \in \mathbb{R}^3$.


\parbf{\ref{ex:scaled-curvature}.} Observe that $\alpha (t) :=  \gamma_{\lambda} (t/\lambda )$ is a unit-speed parameterization of the curve $ \gamma_{\lambda}$. Apply the chain rule twice.
...
\parit{Case $|a|>1$,} Note that $\tan_a(t)=\gamma'_a/|\gamma'_a|$ runs clockwise and makes full turn in time $2\cdot\pi$.
...
See the drawings; we assume that the two marked points in the last example have parallel tangent lines.
...
Use tha $\gamma_t$ moves almost horizontally to the right to show that in a small neighborhood of $p$, the curve $\gamma_1$ lies above $\gamma_0$.
...
Further, show that the map $(x,y,z)\mapsto (\tfrac{2\cdot x}{x^2+y^2+z^2},\tfrac{2\cdot y}{x^2+y^2+z^2})$
describes the inverse map which is continuous away from the origin.
...
Therefore, in order to construct a diffeomorphism from one open subset of the plane to another it sufficient to construct a smooth vector field such that its flow maps one set to the other;
...
but be careful with with the cases \ref{SHORT.ex:star-shaped-disc:nonsmooth} and \ref{SHORT.ex:star-shaped-disc:star-shaped} --- they are not as easy as one might think.
...
\parit{Remark.}
The exercise can be reduced to a deeper result of Heinz Hopf and Willi Rinow \cite{hopf-rinow}, stating that \emph{if Gauss curvature of $\Sigma$ is at least $1$,
then
...
(This result named for Sumner Myers who generalized it latter \cite{myers}.) 
...
Denote by $\vec u_n$ the the unit vector in the direction from $x_n$; that is $\vec u_n=\tfrac{x_n}{|x_n|}$.
...
\parit{\ref{SHORT.ex:convex-proper-plane:b} and \ref{SHORT.ex:convex-proper-plane:c}.}
...
\parit{\ref{SHORT.ex:convex-proper-plane:d}.} Follows from \ref{SHORT.ex:convex-proper-plane:b} and \ref{SHORT.ex:convex-proper-plane:c}.
...
In the remaining case, if $f(x_n,y_n)\to -\infty$, use convexity of $f$ to show that $f(\tfrac{x_n}2 ,\tfrac{y_n}2)\z\to -\infty$.
Show that $(\tfrac12\cdot x_\infty, \tfrac12\cdot y_\infty)\in\Omega$, and
