...
We present an elementary argument.
...
\begin{itemize}
 ...
 Then, passing to a smaller interval $\mathbb{J}\ni0$ if necessary, we can find a unique smooth function $r$ such that $h=r^2$, $r(0)=0$ and $r'(0)> 0$.
\end{itemize}
Note that by passing to a smaller rectangular domain of $f$ we can assume that $f_{xx}>\eps$ and $f_{yy}<-\eps$ for a fixed $\eps>0$. 
Show that we can assume that for every $x$ there is unique $y(x)$ such that $f_x(x,y(x))=0$; 
moreover the function $x\mapsto y(x)$ is smooth.

Set $h(x)=f(x,y(x))$.
Note that $h(0)=h'(0)=0$ and $h''>0$.
Applynig the claim, we get that $h(x)=-s^2(x)$, where $s(0)=0$ and $s'(0)>0$.

Observe that $g(x,y)=h(x)-f(x,y)\ge 0$ and $g_y(x,y(x))=g(x,y(x))=0$ and $g_{yy}>0$.
Applying the claim for each functions $y\mapsto g(x,y)$ with fixed $x$, we get that $g(x,y)=r(x,y)^2$ for a smooth function $r$ such that 
$r(x,y(x))=0$ and $r_y(x,y(x))>0$.

It follows that 
\begin{align*}
f(x,y)&=s(x)^2-r(x,y)^2-=
\\
&=(s(x)-r(x,y))\cdot (s(x)+r(x,y)).
\end{align*}
That is $f(x,y)=0$ if $s(x)\pm r(x,y) =0$.
It remains to that the two functions $g_\pm(x,y)=s(x)\pm r(x,y)$ have distinct non-zero gradients at $0$ and apply the implicit function theorem.
...
\parbf{\ref{ex:K=1}.} Modify the proof of \ref{prop:rauch} showing that $K\equiv 1$ implies that $b(\theta,r)=\sin r$ for all small $r\ge 0$.
