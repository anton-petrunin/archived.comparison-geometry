...
Recall that the function $f$ is smooth; see Section\nobreakspace\ref{sec:analysis}.
...
Use it to show that $L$ is the range of\nobreakspace$\alpha$.
...
Now suppose $\beta\:t\mapsto (x(t),y(t))$ is a smooth parametrization of\nobreakspace$L$.
...
Observe that $\measuredangle(\gamma'(s_0),\gamma'(s))\le \theta$ for any\nobreakspace$s$.
...
so by the definition (Section\nobreakspace\ref{sec:def-smooth-surface}) $\Sigma_0$ is not a smooth surface.
...
Without loss of generality we can assume that the origin belongs to both $\Sigma$ and\nobreakspace$\Theta$.
...
and therefore $\gamma$ is a curvature line in\nobreakspace$\Sigma_2$.
...
Show that $L$ is a smooth surface and the closest point projection $L\to \Sigma$ is a smooth regular parametrization of\nobreakspace$\Sigma$.
...
By \ref{thm:convex-embedded}, the surface bounds a convex region that contains a line segment of length\nobreakspace$\pi$.
...
The latter means that any two points in $\Sigma$ can be connected by a path that lies in $\Sigma$ and has length at most\nobreakspace$\pi$.
...
see Section\nobreakspace\ref{sec:implicit-curves}.
...
Show that $s$ is a diffeomorphism from $F_\epsilon$ to the south hemisphere of\nobreakspace$\Sigma$.
...
Without loss of generality, we can assume that $p$ is the origin of\nobreakspace$\mathbb{R}^3$.
...
One can use the cutoffs and mollifiers to do this (Section\nobreakspace\ref{sec:analysis}).
