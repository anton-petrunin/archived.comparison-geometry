...
The solutions of these exercises are build on the following general construction.
...
more over the same holds for its inverse since.
The map $\iota$ is called \emph{flow} of vector field $\vec u_t$.

Therefore, in order to construct a diffeomorphism from one open subset of the plane to another it sufficient to construct a smooth vector field such that iots flow maps one set to the other;
such map is automatically a diffeomorphism.


\parit{\ref{SHORT.ex:plane-n}.}
Let us show first that there is a diffeomorphism of the plane to itself that moves a given point $x$ to an arbitrary point $y$ in its $\eps$-neighborhood and does not move points outside of $2\cdot\eps$-neighborhood of $x$.

Choose a smooth function $f\:\RR^2\to [0,1]$ that takes value $1$ in the $\eps$-neighborhood of $x$, vanish outside of $2\cdot\eps$-neighborhood.
Choose a constant vector field $\vec v$ on $\RR^2$ such that 
$y=x+\vec v$.
Consider the vector field $\vec u=f\cdot \vec v$.
For any point $s\in \RR^2$ consider the solution $t\mapsto s(t)$ of ODE 
$s'(t)=\vec u(s(t))$ with the initial value $s(0)=s$.
Consider the map $\iota\:s\mapsto s(1)$.
Observe that $\iota(x)=y$ and that $\iota$ does not move points outside of $2\cdot\eps$-neighborhood of $x$.
Use \ref{thm:ODE} to show that $\iota\:\RR^2\to\RR^2$ is diffeomorphism and make a conclusion.

By composing finitely many such diffeomorphisms one can move the deleted points arbitrary (while kipping them distinct).
Hence the statement follows. 

\parit{\ref{SHORT.ex:star-shaped-disc:smooth}.}
Without loss of generality we can assume that the origin belongs to both $\Sigma_1$ and $\Sigma_2$.
...
A diffeomorphsm can be found among the maps that can be written in palar coordinates as $(\theta,\rho)\mapsto  (\theta,f(\theta,\rho))$ for a smooth function $f$ of two variables.
???
...
Then homeomorphism to the plane can be described in polar coordinate for example as 
$(\theta,r)\mapsto (\theta,
...
\parbf{\ref{ex:intK}}; \ref{SHORT.ex:intK:4pi}
