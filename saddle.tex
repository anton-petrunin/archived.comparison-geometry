...
Assume $R$ is a convex body in $\RR^3$ bounded by a surface with principal curvatures at most 1.
Show that $R$ contains a unit ball.
\end{thm}

Recall that a region $R$ in the Euclidean space is called  \index{strictly!convex}\emph{strictly convex} if for any two points $x,y\in R$, any point $z$ between $x$ and $y$ lies in the interior of $R$.
...
Let us show that $R$ is \index{locally strictly convex}\emph{locally strictly convex};
...
then the surface is said to be \index{strictly!saddle}\emph{strictly saddle};
...
Let $\Sigma$ be an open strictly saddle surface and $f\:\RR^3\z\to\RR$ be a smooth convex function.
...
such that the intersection of $\Omega$ with the tangent plane $\T_p$ is a union of two smooth curves  \index{intersect transversally}\emph{intersecting transversally} at $p$.
\end{thm}
...
to
\[\Delta_\epsilon=\set{(x,y,f(x,y))\in \RR^3}{f(x,y)\le \epsilon};\]
...
$\DD\to F_\epsilon$, where $\DD\z=\set{(x,y)\in\RR^2}{x^2+y^2\le 1}$ is the unit disc.
\end{thm}
