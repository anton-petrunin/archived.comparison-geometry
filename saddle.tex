...
Show that any closed surface that lies at distance at most 1 from a straight line has a point with Gauss curvature at least\nobreakspace1.
\end{thm}
...
This property implies that $f$ is a strictly convex function in a neighborhood of the origin in the $(x,y)$-plane (see Section\nobreakspace\ref{sec:analysis}).
...
Recall that a set $R$ in the plane is called \index{star-shaped}\emph{star-shaped} if there is a point $p\in R$ such that for any $x\in R$ the line segment $[p,x]$ belongs to\nobreakspace$R$.
...
Then the entire surface $\Sigma$ lies in\nobreakspace$R$.
\end{thm}
...
Denote by $\Sigma'$ the part of $\Sigma$ that lies with $p$ on the same side of\nobreakspace$\Pi$.

Since $\Sigma$ is compact, it is surrounded by a sphere;
let $\sigma$ be the circle of intersection of this sphere and $\Pi$.
Consider the smallest spherical dome $\Sigma_0$ with boundary $\sigma$ that surrounds\nobreakspace$\Sigma'$.

Note that $\Sigma_0$ supports $\Sigma$ at some point\nobreakspace$q$.
...
Assume the boundary line of $\Delta$ lies in the unit sphere centered at\nobreakspace$p$.
