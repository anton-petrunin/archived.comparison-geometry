...
It follows that, we can assume that $\Sigma_1$ and $\Sigma_2$ are cooriented at $p$;
...
we have that $D^2_{\vec w}f(x,y)>0$ for $(x,y)$ in a neighborhood of the origin.
...
Show that any connected component of intersection of $\Sigma$ with a plane is a curve or a single point.
\end{thm}
...
Assume $\Sigma$ is an open or closed smooth surface with positive Gauss curvature.
...
Let $R$ be the region bounded by $\Sigma$ that lies on the side of $\Norm$;
...
Note that the proof above imply the following general statement: \emph{any connected locally convex region is convex}.
...
Let $\Sigma$ be a closed smooth surface of diameter at least $\pi$; that is there is a pair of points $p,q\in\Sigma$ such that $|p-q|\ge \pi$.
Show that $\Sigma$ has a point with arbitrarily small Gauss curvature at most 1.
\end{thm} 
...
\vskip-0mm
\end{wrapfigure} %%%???redo the picture
...
The proof of this statement can be obtained by applying the so called \emph{coarea fromula} together to with the inequality in the exercise. 
...
In particular the set $F_\eps$ defined by the inequality $f(x,y)\le \eps$ is a convex set for sufficiently small $\eps>0$;
