...
Let us use a shortcut notation $\Norm(s)\z=\Norm(\gamma(s))$.
Note that the unit vectors $\tan(s)$ and $\Norm(s)$ are orthogonal.
Тherefore there is a unique unit vector $\mu(s)$\index{$\mu$} such that 
$\tan(s),\mu(s),\nu(s)$ is an oriented orthonormal basis;
it is called \index{Darboux frame}\emph{Darboux frame} of $\gamma$ at $s$.
...
that is,
\[\gamma''(s)=k_g(s)\cdot \mu(s)+k_n(s)\cdot\Norm(s).\]\index{$k_g$}\index{$k_n$}
The values $k_g(s)$ and $k_n(s)$ are called \index{geodesic curvature}\emph{geodesic} and \index{normal curvature}\emph{normal curvature} of $\gamma$ at $s$.
...
By that reason the normal curvature is also denoted by $k_{\vec v}$;\index{$k_{\vec v}$}
...
Show that the sum of the normal curvatures for any pair of orthogonal directions, at a point $p\in\Sigma$ equals to $H(p)$ --- the mean curvature at $p$. 
\end{thm}
...
here $\kur(t_0)$ is the curvature and $k_n(t_0)$ is the normal curvature of $\gamma$ at $t_0$.  
\end{thm}
...
Suppose that $\Sigma$ is the surface of revolution around $x$-axis with generatrix $\gamma$.
...
\[y''=y.\] 
If $y=e^{-s}$, then the surface $\Sigma$ is called \emph{psudosphere}; it is shown on the diagram.
\end{enumerate}
...
Show that \index{catenoid}\emph{catenoid} defined implicitly by equation
\[(\cosh z)^2=x^2+y^2\]
is a minimal surface.
\end{thm}

\begin{thm}{Exercise}\label{ex:helicoid-is-minimal}
Show that \index{helicoid}\emph{helicoid} defined by the following parametric equation
...
Now we are going to show by example that the answer is  ``no'' in general case;
this example was constructed by Vladimir Lagunov \cite{lagunov}.
...
It remains to modify $V_1$ to make its boundary connected without increasing bounds on its principle curvatures and  without allowing larger balls inside.
...
Question \ref{quest:lagunov} can be asked differently: what is the maximal radius $r$ of the ball that has to be included in any body bounded by smooth surface with principle curvatures bounded in absolute value by 1.
...
One may also consider bodies bounded by few smooth surfaces.
In this case the example of a region between two large concentric spheres with almost equal radiuses shows that in general case there is no bound.
Indeed, this region can be made arbitrary thin while the curvature of the boundary can be made arbitrary close to zero.

Recall that Lagunov example shows that $r\le r_2$,
...
The following exercise claims that this bound is optimal.
