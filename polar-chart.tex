...
where $b=b(u,v)\df | s_u|=| s_v|$, and $\triangle$ denotes the \index{laplacian}\emph{laplacian}; that is, $\triangle=\tfrac{\partial^2}{\partial u^2}+\tfrac{\partial^2}{\partial v^2}$.
\end{thm}
...
A map $f\:\Sigma\to \hat{\Sigma}$ is called  \index{length-preserving}\emph{length-preserving} if for any curve $\gamma$ in $\Sigma$ the curve $\hat\gamma=f\circ\gamma$ in $\hat{\Sigma}$ has the same length. 
...
However if the the disc is larger than half-sphere, then the proof is quite hard. %???REF
}
\end{thm}
