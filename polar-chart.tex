...
Suppose that $\Sigma$ has an orientation defined by a unit normal field $\Norm$.
Let us use shortcut $\Norm(u,v)$ for $\Norm(s(u,v))$.
At each point $s(u,v)$ let us consider an orthonormal frame $\Norm$, $\vec u=s_u$ and $\vec v=\Norm\times \vec u$.

Recall that $s_v\perp \vec u$ and $s_v\perp \Norm$.
The latter follows since the vector $s_v(u,v)$ is tangent to $\Sigma$ at $s(u,v)$.
Therefore we have that $s_v=b\cdot\vec v$ for some smooth function $(u,v)\mapsto b(u,v)$.
...
\parit{Proof of $\vec u_u=\ell\cdot \Norm$.} This vector identity can be rewritten as the following three scalar identities:
\[
\begin{aligned}
...
Since $\vec v=\Norm\times \vec u$, we have
\[\begin{aligned}
...
Use \ref{lem:K(orthogonal)} to show that Gauss curvature can be expressed as 
