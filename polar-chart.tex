...
A map $f\:\Sigma\to \hat{\Sigma}$ is called  length-preserving if for any curve $\gamma$ in $\Sigma$ the curve $\hat\gamma=f\circ\gamma$ in $\hat{\Sigma}$ has the same length. %???it is sufficient to consider smooth only curves???
...
Suppose that $\vec u,\vec v$ is smooth oriented orthonormal frame of tangent vectors to $\Sigma$ defined in a neighborhood of $\gamma$;
that is, $|\vec u|=|\vec v|=1$, $\vec u\perp\vec v$ and $\vec u\times\vec v=\Norm$, where $\Norm$ is the unit normal field to $\Sigma$ that defines its orientation.
Denote by $\vec u(t)$ and $\vec v(t)$ the vectors of the fields at $\gamma(t)$.
The \index{rotation}\emph{rotation} of $\vec u$ around $\gamma$ is defined as the integral
\[\int_0^1\langle\vec u'(t),\vec v(t)\rangle\cdot dt.\]

\begin{thm}{Lemma}
Suppose that $\gamma$ is a loop based at a point $p$ in a smooth surface $\Sigma$ and $\vec u,\vec v$ is a smooth oriented orthonormal frame of tangent vectors to $\Sigma$ defined in a neighborhood of $\gamma$.
Let $\phi$ be rotation of $\vec u$ around $\gamma$.
Then the parallel transport $\iota_\gamma\:\T_p\to\T_p$ is a clockwise rotation by the angle $\phi$.
...
First, the following identity in polar coordinates $(r,\theta)\mapsto w(r,\theta)$ on a smooth surface:
\[K=-\frac{b_{\theta\theta}}b,\]
where $b=|w_\theta|$; see \ref{lem:K(orthogonal)}.
...
\[\iint_D (Q_x- P_y)\cdot dx\cdot dy=\int_\gamma (P\cdot dx+Q\cdot dy).\]
