...
\begin{itemize}
\item Proposition~\ref{prop:inj-rad} --- a alternative definitions of injectivity radius.
...
However, according to Proposition~\ref{prop:exp} polar coordinates behave the usual way for small values $r$.
...
Then there is a shortest path from $p$ to $q$, which becomes a geodesic if parametrized by its arc-length.
...
But according to \ref{prop:exp}, the exponential map is injective in the $\epsilon$-neighborhood of zero --- a contradiction.\qeds
...
The map $s$ is called \index{semigeodesic}\emph{semigeodesic} if for any fixed $v$ the $u\mapsto s(u,v)$ is a unit-speed geodesic and $s_u\perp s_v$ for any $(u,v)$.
...
$\langle\vec u_u,\Norm\rangle=\ell$ follow from \ref{thm:shape-chart} and \ref{eq:Shape(u,v)}.
Indeed
\begin{align*}
...
Consider one-parameter family $\Sigma_t$ of graphs $z=t\cdot f(x,y)$ and denote by $\Delta_t$ the corresponding disc in $\Sigma_t$, so $\Delta_1=\Delta$ and $\Delta_0$ is its projection to the $(x,y)$-plane.
...
A simple example of intrinsic isometry can be obtained by warping a plane into a cylinder.
...
Taking this into account, the second identity holds since otherwise $f$ does not preserve length of coordinate lines $\gamma\:t\z\mapsto s(t,c-t)$ for some constant~$c$.
