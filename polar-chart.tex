...
\begin{itemize}
\item Proposition\nobreakspace\ref{prop:inj-rad} --- an alternative definition of injectivity radius.
\item Proposition\nobreakspace\ref{prop:gamma''} --- shortest paths are geodesics.
...
Namely, fix some polar coordinates $(r,\theta) \in \mathbb{R}^+ \times \mathbb{S}^1  $ on the tangent plane $\T_p$.
If $\vec v\in \T_p$ has coordinates $(r,\theta)$,
then we say that $s(r,\theta)=\exp_p\vec v$ is the point in $\Sigma$ with  polar coordinates $(r,\theta)$.


Since there might be many geodesics from $p$ to a given point $x$,
the point $x$ might have many different polar coordinates.
However, according to Proposition\nobreakspace\ref{prop:exp} polar coordinates behave in  the usual way for small values of $r$.
...
The following statement will play a key role in the formal proof that shortest paths are geodesics; see Section\nobreakspace\ref{sec:proof-of-gamma''}.
...
In this section we use the construction of polar coordinates and the Gauss lemma \ref{lem:palar-perp} to prove Proposition\nobreakspace\ref{prop:gamma''}.
...
By\nobreakspace\ref{adex:integral-length:a}, we have that
\begin{align*}
...
Note that we can choose a unit vector field\nobreakspace$\Norm=\Norm(u,v)$ that is normal to $\Sigma$ at the point $s(u,v)$.
...
Let $\Sigma$ be a smooth oriented surface and $\gamma$ a simple closed path in\nobreakspace$\Sigma$.
...
Assume that $\Delta$ lies in a local graph realization $z=f(x,y)$ of\nobreakspace$\Sigma$.
...
Since  any point of $\Sigma$ has a neighborhood that can be covered by such a graph, applying Lemma\nobreakspace\ref{lem:GB-sum} as above we get that \ref{eq:GB=0} holds for any disc\nobreakspace$\Delta$ in\nobreakspace$\Sigma$.
\qeds
...
Taking this into account, the second identity holds since otherwise $f$ would not preserve the lengths of the coordinate lines $\gamma\:t\z\mapsto s(t,c-t)$ for some constant\nobreakspace$c$.
