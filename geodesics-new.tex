...
A rigorous proof will be given in the sections \ref{sec:exp}--\ref{sec:proof-of-gamma''}, but the following informal physical explanation might be convincing.
...
The Jacobian of $\exp_p$ at zero is the identity matrix.
\end{thm}

\parit{Proof.} Let $z=f(x,y)$ be a local graph representation of $\Sigma$ in the tangent-normal coordinates.
...
$(0,1,0)$
correspondingly.
A general tangent vector can be written as ${\vec v}=(x,y,0)$.
...
\end{pmatrix}.
\]
It follows that the Jacobian matrix of the projection of $\exp_p$ to the $(x,y)$-plane is the identity matrix.
\qeds %???Jacobian matrix def
