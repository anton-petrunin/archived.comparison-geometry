...
Show that if a curve that is a geodesic and an asymptotic line on a smooth surface,
then the curve is a line segment.
\end{thm}
...
lemma states the conservation of energy and the proposition gives smooth dependence of trajectory of a particle depending on its initial position and velocity.
...
Moreover
\begin{enumerate}[(a)]
\item\label{prop:geod-existance:smooth} the map $(p,{\vec v},t)\mapsto \gamma(t)$ is smooth in its domain of definition.
\item\label{prop:geod-existance:whole} if $\Sigma$ is proper, then $\mathbb{I}=\RR$; that is, the maximal interval is whole real line.
\end{enumerate}

\end{thm}

This proposition follows from the theorem on existance and uniqueness of the initial value problem (\ref{thm:ODE})
but the proof contains few technical details, so we decided to write it separately in Section~\ref{sec:geod-existance}.
...
The following proposition provides connection between intrinsic geometry of the surface and its extrinsic geometry.
...
A rigorous proof will be given in the next few sections.
Before that, let us give an informal physical explanation why $\gamma''(t)\perp\T_{\gamma(t)}\Sigma$.
(In fact, if one assumes that $\gamma$ is smooth, then it is easy to convert this explanation into a rigorous proof.)

One may think about a shortest path $\gamma$ as of stable position of a stretched elastic thread that is forced to lie on a frictionless surface.
