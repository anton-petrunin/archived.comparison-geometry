...
%???if we add that perpendicular bisector to [\bar p,p] separates R from p, then we can remove separation lemma from the preliminaries.
...
In fact it is not hard to prove that the function {}\emph{$p\mapsto \inj(p)$ is lower semicontinuous and positive}. %???ref
