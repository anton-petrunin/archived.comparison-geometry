...
(In other words, if $s\:\RR^2\to \Sigma$ is the natural bi-periodic parameterization of $\Sigma$, then $s\circ\gamma$ is not a geodesic for any closed curve $\gamma$ in $\RR^2$.)
\end{thm}
...
that is, for any $q\in \Sigma$ there is $\eps>0$ such that if $|q-p|_\Sigma<\eps$, then we can assume that $r_p\ge\eps$.
\end{thm}
...
In fact a stronger statement holds: \emph{the function $p\mapsto \inj(p)$ is lower semicontinuous and positive}, but we will not use it in the sequel. %???REF
