...
\parbf{Nonexistence.} Let us give an example of a surface with a pair of points without shortest path.
...
Without loss of generality, we may assume that $\length\gamma_n<\ell+1$ for any $n$ and each $\gamma_n$ is parameterized proportional to its arclength.
...
By \ref{lem:closest-point-projection} $|p_i-p_{i-1}|\ge|\bar p_i-\bar p_{i-1}|$ for any $i$.
Therefore 
...
The suggested argument does not give the optimal bound the Lipschitz constant that guarantee that $\gamma$ is simple, but
later we will give the exact bound. %+??? ref


\begin{thm}{Advanced exercise} Suppose a smooth surface $\Sigma$ bounds a convex set $K$ in the Euclidean space.
...
\parit{Hint:} Use Exercise~\ref{ex:intrinsic-diameter}.
