...
%moreover, 
%\emph{if there is a point $p\in \Sigma$ such that for any tangent vector ${\vec v}\in\T_p\Sigma$ there is a two side infinite geodesic $\gamma$ with $\gamma'(0)={\vec v}$, then $\Sigma$ is proper.}

%Cannot be proper. z=sin(1/x), x>0 is a counterexample.
...
that is, for any $q\in \Sigma$ there is $\epsilon>0$ such that if $|q-p|_\Sigma<\epsilon$, then $r_p\ge\epsilon$.
\end{thm}
...
As one can see from the previous exercise, a geodesics might fail to be shortest paths. A geodesic that is also a shortest path is called a  \index{minimizing geodesic}\emph{minimizing geodesic}.
...
\[\tc\gamma\le \frac{100}{\epsilon^2}.\]
\end{subthm}

\end{thm}

Note that the obtained bound of the total curvature goes to infinity as $\epsilon\to 0$.
In fact there is a bound that is independent of $\epsilon$ \cite{lebedeva-petrunin}.
