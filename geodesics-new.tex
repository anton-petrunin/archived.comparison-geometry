...
A smooth curve $\gamma$ in a smooth surface $\Sigma$ is called a \index{geodesic}\emph{geodesic} if for any\nobreakspace$t$, the acceleration $\gamma''(t)$ is perpendicular to the tangent plane $\T_{\gamma(t)}$.
...
Show that $\gamma$, parametrized by arc-length, is a geodesic on\nobreakspace$\Sigma$.
\end{thm}
...
Given a tangent vector ${\vec v}\in \T_p$, consider a geodesic $\gamma_{\vec v}$ in $\Sigma$ that starts at $p$ with initial velocity\nobreakspace$\vec v$; 
that is, $\gamma(0)=p$ and $\gamma'(0)={\vec v}$.
...
A formal proof of the proposition will be given later; see Section\nobreakspace\ref{sec:proof-of-gamma''}.
...
Since it is frictionless, the force density $\vec n=\vec n(t)$ that keeps $\gamma$ in the surface must be proportional to the normal vector to the surface at\nobreakspace$\gamma(t)$.
...
Assume $\gamma$ is a unit-speed geodesic on a smooth convex surface $\Sigma$ and a point $p$ lies in the interior of the convex set bounded by\nobreakspace$\Sigma$.
...
Then the total curvatures of any shortest path in $\Sigma$ can be bounded in terms of\nobreakspace$\epsilon$. 
\end{thm}
...
According to Exercise\nobreakspace\ref{ex:tc-spherical-image}, this statement would also follow if the length of the spherical image of $\gamma$ could be bounded above; 
