...
moreover, if $\Sigma$ is complete, then $\exp_p$ is defined on the whole space $\T_p$.
...
This connection will be important later; in particular it will play the key role in the proof of the so-called \emph{remarkable theorem}. %???+ref(see \ref{thm:remarkable}).
...
later (see \ref{ex:sqrt(3)}) we will show that the exact bound is $\sqrt{3}=\tan\tfrac\pi3$ --- it is the same as in the exercise about mountain of with the shape of a perfect cone; see \ref{ex:lasso}.
