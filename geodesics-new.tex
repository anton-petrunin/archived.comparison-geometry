...
The map $p\mapsto \bar p$ is called the \index{closest point projection}\emph{closest point projection};
...
then the angle $p \bar p \bar q$ is right or obtuse.
...
The same way we can show that  if $q\z\ne \bar q$, then the angle ${q}{\bar q}{\bar p}$ is right or obtuse.
...
The point $q=\gamma_{\vec v}(1)$ is called \index{exponential map}\emph{exponential map} of ${\vec v}$, or briefly $q=\exp_p{\vec v}$.
