...
The latter statement is a part of the \index{Hopf--Rinow theorem}\emph{Hopf--Rinow theorem} \cite{hopf-rinow} which provides a converse;
moreover, 
...
that is, for any $q\in \Sigma$ there is $\eps>0$ such that if $|q-p|_\Sigma<\eps$, then $r_p\ge\eps$. %???r_p -->r???
\end{thm}

%???(a)+(b)about diffeomorphism + it has to be proved in semigodesic chart.

Given $p\in \Sigma$, the least upper bound on $r_p$ that satisfies the proposition is called \index{injectivity radius}\emph{injectivity radius} of $\Sigma$ at $p$;
it is denoted by $\inj(p)$.
The proposition states that {}\emph{injectivity radius is positive and locally bounded away from zero}.

\parbf{Remarks.}
The proofs of the following two statements are not hard,
but we omit them since we will not use them in the sequel.

\begin{itemize}
\item {}\emph{The function $p\mapsto \inj(p)$ is lower semicontinuous and positive}.
\item \emph{If $\exp_p$ is injective in $B(0,r_p)_{\T_p}$, then the restriction $\exp_p|_{B(0,r_p)}$ is a diffeomorphism to its image}.
In other words, the injectivity radius could be defined as the least upper bound on $r_p$ such that $\exp_p$ is injective in the ball $B(0,r_p)_{\T_p}$.
\end{itemize}
%???REF+remove

The proof of the proposition uses the observation and the inverse function theorem (\ref{thm:inverse}). %???+Jacobian matrix
...
\begin{thm}{Corollary}
Let $\Sigma$ be a smooth regular surface, $p\in\Sigma$ and $r\le \inj(p)$.
