...
it is not be used in the sequel.
...
The second one is proved by taking derivative of the identity $\langle \norm,\norm\rangle\z=1$ in \ref{eq:orthornomal}. 
...
\item\label{ex:spherical-frenet:kur-tor} Assume that the torsion of $\gamma$ does not vanish.
Use (\ref{ex:spherical-frenet:tau})--(\ref{ex:spherical-frenet:beta}) to show that 
\[\left|\frac{\kur'}{\tor}\right|=\kur\cdot\sqrt{\kur^2-1}.\]
(It proves the ``only if'' part of the theorem.)
