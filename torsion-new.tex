...
The following theorem was proved by Michel Ange Lancret\nobreakspace\cite{lancret} more than two centuries ago.
...
Use \ref{SHORT.ex:spherical-frenet:beta} to show that if $\gamma$ is closed, then $\tor(s)=0$ for some\nobreakspace$s$.
\end{subthm}
...
By Theorem\nobreakspace\ref{thm:ODE}, at least one of the values $\gamma(s)$, $\tan(s)$, $\norm(s)$, $\bi(s)$
