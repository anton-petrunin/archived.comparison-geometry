...
\end{thm}%??? exact bound if $\tfrac\pi2\cdot D$
...
In the proof we will use the following claim; we do not prove it, but make a physical interpretation which might help to believe it.
...
This claim provides connection between intrinsic geometry of the surface and its extrinsic geometry which will be important latter.
...
This connection will play an important role in the proof of the remarkable theorem (see \ref{thm:remarkable}).

For instance geodesic $\gamma$ is an object of intrinsic geometry of the surface $\Sigma$,
while its second derivative $\gamma''$ is not.
