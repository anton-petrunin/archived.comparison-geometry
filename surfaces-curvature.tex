...
If $w$ is unit, then the second directional derivative $D^2_wf(0,0)$ can be intepreted as the signed curvature of the curve formed by the intersection of $\Sigma$ with the plane thru $p$ spanned by $\Norm_p$ and $w$.
...
assuming that the $z$-axis points in the direction of the unit normal vector $\Norm_p$ to both surfaces.
...
Consider the plane $\Pi$ passing thru $p$ and spanned by the normal vector $\Norm_p$ and $w$.
Let $\gamma_1$ and $\gamma_2$ be the curves of intersection of $\Sigma_1$ and $\Sigma_2$ with $\Pi$.

Let us orient $\Pi$ so that $\Norm_p$ points to the left from $w$.
...
\[\langle \gamma''(t),\norm_{\gamma(t)}\rangle=\II_{\gamma(t)}(\gamma'(t),\gamma'(t)).\]
...
\parit{Proof.}
Denote by $\Norm$ the unit normal vector to $\Sigma$ at $p$ 
and by $\norm$ the unit normal vector in the Frenet frame of $\gamma$.
Note that $\cos\alpha=\langle\Norm,\norm\rangle$.

Applying \ref{prop:gamma''=II}, we get that
\begin{align*}
k_{w}(p)&=\II_p(w,w) =
\\
&=\langle\gamma'',\Norm\rangle=
\\
