...
Consider a coordinate system $(x,y,z)$ with origin at $p$ such that the $(x,y)$-plane coincides with $\T_p$ and the $z$-axis points in the direction of the normal vector $\Norm_p$.
...
gives the best approximation of the surface at $p$ among all paraboloids;
...
\eqlbl{eq:hessian}
\]
$M_p$ is called the \index{Hessian matrix}\emph{Hessian matrix} of $f$ at $(0,0)$.\index{10m@$M_p$}
...
\[M_p=\begin{pmatrix}
  ...
  \end{pmatrix},
\]
the diagonal components $k_1$ and $k_2$ of $M_p$ are called the \index{principle curvatures}\emph{principle curvatures} of $\Sigma$ at $p$;\index{10k@$k_1$, $k_2$}
...
The eigendirections of $M_p$  are called the \index{principle directions}\emph{principle directions} of $\Sigma$ at~$p$.
...
Since the product of the eigenvalues of a matrix equals its determinant,
the Gauss curvature equals the determinant of the Hessian matrix 
$M_p\z=
(\begin{smallmatrix}
\ell&m
\\
...
The sum \index{10h@$H$}
\[H(p)=k_1(p)+ k_2(p)\] 
is called the \index{mean curvature}\emph{mean curvature}\footnote{Some authors define the mean curvature as $\tfrac12\cdot(k_1(p)+ k_2(p))$ --- the mean value of the principle curvatures. It suits the name better, but it is not as convenient when it comes to calculations.} at $p$.
...
In particular, the Gauss curvature is well defined at every point for a nonoriented surface $\Sigma$.
...
Let $p$ be a common point of two surfaces $\Sigma_1$ and $\Sigma _2$. We define the \textit{angle of intersection} of $\Sigma_1$ and $\Sigma_2$ at $p$ as the acute angle between the normal lines to the tangent planes $T_p\Sigma_1$ and $T_p\Sigma_2$. The following exercise is a result by Ferdinand Joachimsthal \cite{joachimsthal} generalized by Ossian Bonnet \cite{bonnet}.

\begin{thm}{Exercise}\label{ex:shape-curvature-line}
Assume that two smooth surfaces $\Sigma_1$ and $\Sigma_2$ intersect at constant angle along a smooth regular curve $\gamma$.
...
Show that the set of points of the form $p+t\cdot \Norm(p)$ with $p\in\Sigma$ forms a smooth surface, say $\Sigma_t$.
\end{subthm}

\begin{subthm}{ex:equidistant:area}
Let $a(t)$ denote the area of $\Sigma_t$.
Show that 
\[a'(0)=-\iint_\Sigma H,\]
