...
Note that $\T_p\Sigma=\T_{\Norm(p)}\SS^2$ --- both of these spaces are normal subspaces to $\Norm(p)$.
...
Let us denote by $\vec i$, $\vec j$ and $\vec k$ the standard basis in the $(x,y,z)$-coordinates.\index{10i@$\vec i$, $\vec j$ and $\vec k$}
...
In particular, the principle curvatures of $\Sigma$ at $p$ are the eigenvalues of $\Shape_p$ and the principle directions are the eigendirections of $\Shape_p$.
\end{thm}

Since Hessian matrix is symmetric, the corollary also implies that $\Shape$ is self-adjoint which gives another way to solve \ref{ex:self-adjoint}.
