...
The eigendirections of $M_p$ are called the {}\emph{principal directions} of $\Sigma$ at\nobreakspace$p$.
...
Suppose that $\Sigma$ and $\Pi$ intersect along a smooth regular curve\nobreakspace$\gamma$.
...
Recall that the components $\ell$, $m$, and $n$ of the Hessian matrix are defined in Section\nobreakspace\ref{sec:lmn}.
...
then $\gamma$ is a curvature line of\nobreakspace$\Sigma$.
\end{thm}
...
As usual we denote by $\Norm$ the unit normal field on\nobreakspace$\Sigma$.
...
that is, $\alpha$ is the angle between the unit normal to $\Sigma$ at $p$ and the unit normal vector in the Frenet frame of $\gamma$ at\nobreakspace$t_0$.
...
(To make the curves smooth, one has to use cutoffs and mollifiers from Section\nobreakspace\ref{sec:analysis}.)
...
We assume that the curves have curvature at most\nobreakspace1.
...
Assuming that the holes are large, its boundary can be made so that its principal curvatures are still at most $1$; the latter can be proved in the same way as for\nobreakspace$V_1$.
\qeds
...
Show that $V$ contains a ball of radius\nobreakspace$r_3$.
