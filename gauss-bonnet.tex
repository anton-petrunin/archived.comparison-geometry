...
A formal computational proof will be given in Section\nobreakspace\ref{sec:gauss--bonnet:formal}.
...
Let us subdivide $\partial \Delta$ into two curves $\gamma_1$ and $\gamma_2$ that share  endpoints with $\delta$ so that
 $\Delta_i$ is bounded by the arc $\gamma_i$  and\nobreakspace$\delta$ for $i=1,2.$
...
In other words, $\GB(\Delta)=0$ for any disc\nobreakspace$\Delta$ in the plane.
...
By Jordan's theorem (\ref{thm:jordan}), $\ell$ bounds a disc in $\Sigma$; denote it by\nobreakspace$\Delta$.
