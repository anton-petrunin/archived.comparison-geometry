...
\section{Buseman function}

A unit-speed geodesic $\gamma\:[0,\infty)\to \spc{X}$ is called a \index{half-line}\emph{half-line} if it is minimizing on each interval $[a,b]\subset [0,\infty)$. 


\begin{thm}{Proposition}\label{prop:busemann}
Suppose $\Sigma$ is a smooth regular surface and $\gamma\:[0,\infty)\to \spc{X}$ is a half-line. 
Then the \index{Busemann function}\emph{Busemann function} $\bus_\gamma\:\Sigma\z\to \RR$ 
\[\bus_\gamma(x)=\lim_{t\to\infty}\dist{\gamma(t)}{x}{\Sigma}- t\eqlbl{eq:def:busemann*}\]
is defined
and $1$-Lipschitz.


\begin{subthm}{prop:busemann-}
If $\Sigma$ is an open simply connected surface with nonpositive Gauss curvature, then $\bus_\gamma$ is convex;
that is, for any geodesic $\alpha$ the real-to-real function 
$t\mapsto \bus_\gamma\circ\alpha(t)$ is convex.
\end{subthm}

\begin{subthm}{prop:busemann+}
If $\Sigma$ is an open surface with nonnegative Gauss curvature, then $\bus_\gamma$ is concave;
that is, for any geodesic $\alpha$ the real-to-real function 
$t\mapsto \bus_\gamma\circ\alpha(t)$ is concave.
\end{subthm}
...
Clearly $\dist{\gamma(t)}{x}{\Sigma}- t\ge-\dist{\gamma(0)}{x}{}$;
that is, for each $x$, the values $\dist{\gamma(t)}{x}{\Sigma}- t$ bounded below.
Thus the limit in \ref{eq:def:busemann*} is defined.

Observe that each function $x\mapsto \dist{\gamma(t)}{x}{\Sigma}- t$ is 1-Lipschitz.
Therefore its limit $x\mapsto\bus_\gamma(x)$ is Lipschitz as well.

It remains to prove the last two statements.
Choose a geodesic~$\alpha$.
Given $\tau\ge 0$, consider the the function $h_\tau(t)=|{\gamma(\tau)}-{\alpha(t)}|_{\Sigma}^2-t^2$.


Observe that $\frac{|\gamma(\tau)-x|}{\tau}\to 1$ for any fixed $x\in\Sigma$ as $\tau\to\infty$.
Therefore
\begin{align*}
\bus_\gamma\circ\alpha(t)
&=
\lim_{\tau\to\infty}\frac{h_\tau(t)}{\tau}
\end{align*}
According to \ref{ex:geod-convexity}, 
$t\mapsto h_\tau(t)$ is convex or concave the function assuming the conditions in  \ref{SHORT.prop:busemann-} or \ref{SHORT.prop:busemann+} respectively.
...
\begin{subthm}{}
Show that a half-line starts at $p$. 
\end{subthm}

\begin{subthm}{}
Suppose $\Sigma$ has nonnegative Gauss curvature at any point.
Consider the function
\[f(x)=\inf_\gamma\bus_\gamma(x),\]
where the infimum is taken for all half-lines $\gamma$ that stat at $p$.
Show that $f$ is concave function and its supsets 
...
A line in a smooth regular surface $\Sigma$ is defined as a both side infinite geodesic $\gamma\:\RR\to\Sigma$ that minimze on each interval $[a,b]\subset \RR$.
...
\parit{Proof.} Consider two Busemann's functions $\bus_+$ and $\bus_-$ asociated with half-lines $\gamma:[0,\infty)\to \Sigma$ and $\gamma:(-\infty,0]\to \Sigma$; that is,
\[
\bus_\pm(x)
=
...
It follows that each level set 
\[L_a=\set{x\in\Sigma}{\bus_+(x)=a}\]
is geodesic;
that is, if a geodesic $\alpha$ has two common points with $L_a$, then $\alpha$ lies in $L_a$.
In other words, each level set $L_a$ is either a closed or both sides infinite geodesic.
