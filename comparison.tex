...
it is called \index{model angle}\emph{model} or \index{comparison angle}\emph{comparison angle} of the triangle $[xyz]$ at $x$.
...
Mikhael Gromov \cite[p.\nobreakspace119]{gromov-1987},
...
The proof of part \ref{SHORT.thm:comp:cat} will be given at the end of Section\nobreakspace\ref{sec:nonpos-comp}.
The proof of part \ref{SHORT.thm:comp:toponogov} will be finished in Section\nobreakspace\ref{sec:nonneg-comp}.
...
The following comutational exercise plays a role in the proof of lemma.

\begin{thm}{Exercise}\label{ex:s-r}
\begin{subthm}{ex:s-r:convex}
Let $\vec u$ be a unit vector in the plane.
Show that for any vector $
\vec w$, the function
\[t\mapsto t+|\vec w|-|\vec w+t\cdot \vec u|\]
is concave.
\end{subthm}

\begin{subthm}{ex:s-r:s-r}
Suppose that hinges $\hinge xpq$ and $\hinge xpy$ have a common side $[x,p]$ and $[x,y]\subset [x,q]$.
Use part \ref{SHORT.ex:s-r:convex} to show that 
...
By te triangle inequality we have 
\[
...
Assume the sequence $(x_n)$ does not terminate.
By \ref{3|xx'|}, we have 
\[
...
\eqlbl{eq:|x-x|><l}
\]
Evidently $\dist{x_n}{p}{},\dist{x_n}{q}{}<
\ell$ for any $n$.
Applying \ref{ex:s-r:s-r} for the hinge $\hinge{x_n}pq$ and $\hinge{x_n}p{x_{n+1}}$ (or $\hinge{x_n}{x_{n+1}}q$, depending on $n$) we get that 
\[r_n-s_n\le 100\cdot (r_n-r_{n+1})\]

The sequences $(r_n)$ and $(s_n)$ are nonincreasing and nonnegative.
Therefore they have to convege.
...
Consider the part of the geodesic $\gamma$ without the arc\nobreakspace$a$.
...
both functions $\bus_\pm$ are concave and $\bus_\pm\circ\lambda(t)=\mp t$ for any\nobreakspace$t$.
...
Whence \ref{clm:parallel} follows for any pair $s$ and\nobreakspace$t$.
