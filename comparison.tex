...
Suppose $\Sigma$ is a smooth regular surface and $\lambda\:[0,\infty)\to \spc{X}$ is a half-line. 
Then 
\[\bus_\lambda(x)=\lim_{t\to\infty}\dist{\lambda(t)}{x}{\Sigma}- t\eqlbl{eq:def:busemann*}\]
is defined $1$-Lipschitz function such that
...
that is, for any geodesic $\alpha$ the real-to-real function 
$t\mapsto \bus_\lambda\circ\alpha(t)$ is concave.
\end{subthm}
...
Intuitively the Busemann function $\bus_\lambda$ can be described as a distance function to the ideal point on the end of the half-line $\lambda$.
...
Observe that $\frac{|\lambda(t)-x|}{t}\to 1$ for any fixed $x\in\Sigma$ as $t\to\infty$.
Therefore
\begin{align*}
...
Moreover, if $K\ni p$ is a noncompact closed convex subset of $\Sigma$, then there is a half-line of $\Sigma$ that starts at $p$ and tuns in $K$.
\end{subthm}
...
where the infimum is taken for all half-lines $\lambda$ that stat at $p$.
...
A line in a smooth regular surface $\Sigma$ is defined as a both side infinite geodesic $\lambda\:\RR\to\Sigma$ that minimze on each interval $[a,b]\subset \RR$.
...
\parit{Proof.} The proof will be divided into steps.
...
\bus_\pm(x)
=
\lim_{t\to\infty}\dist{\lambda(\pm t)}{x}{}- t.
\]
...
\]
for any $x\in \Sigma$.
In particular, by \ref{prop:busemann} we get $\bus_\pm\circ\lambda(t)=\mp t$ for any~$t$.

Fix $x\in \Sigma$.
Since $\lambda$ is a line, we have that 
\[\dist{\lambda( t)}{x}{}+\dist{\lambda(- t)}{x}{}\ge 2\cdot t.\]
...
is concave.
In particular, 
\[|\lambda(t)-x|\le \sqrt{t^2+at+b}\]
for some constants $a,b\in\RR$. 
Passing to the limit as $t\to\pm\infty$, we get \[\bus_+(x)+\bus_-(x)\le0;\]
whence \ref{eq:bus+-=0} follows.

\parit{Step 2.}
Let us show that, the surface $\Sigma$ can be subdivided into lines that run in the direction of gradient of $\bus_-$.
...
denote by $\vec w^a\in\T_x$ the unit vector in the direction at $x$ of geodesic $[x\,\lambda(a)]$. 
...
\\
&=
\limsup_{a\to\infty}\frac{\bus_-\circ\lambda(a)-\bus_-(x)}{|x-\lambda(a)|_\Sigma}\ge
\\
&\ge\limsup_{a\to\infty} \frac{a-\bus_-(x)}{a+|x-\lambda(0)|_\Sigma}=
\\
&=1.
\end{align*}
...
\intertext{and analogously, we get}
\lim_{a\to-\infty}\measuredangle(\vec u,\vec w^a)&=\pi.
\end{align*}

Set $b=\bus_-(x)$.
Let us shoot a unit-speed geodesic $\zeta$ from $x$ in the direction of $\vec u$; we can assume that $\zeta(b)=x$.
...
&=|t_1-t_0|.
\end{align*}
Whence $\zeta$ is a line for any $x$ and $\zeta$ always runs in the direction of $\vec u$.

\parit{Step 3.}
Let us show that the distances between points on two lines in the direction of $\vec u$ behave the same way as the distances between parallel lines in Euclidean plane; see \ref{clm:parallel}.

Let $\xi$ be a line in the direction of $\vec u$; choose 
$x=\xi(b)$ for some $b$.
We assume that $\xi$ is parametrized as above;
in particular $b=\bus_-(x)$.
Let $t\to\delta^x_a(t)$ be the parametrization of $[x\,\lambda(a)]$ by arc length starting from $x$.
Since $\measuredangle(\vec u,\vec w^a)\to 0$ as $a\to\infty$,
...
Since the triangle $[x_0\,\gamma(a)\, z_0]$ is fat (\ref{fat}) we get a lower bound for the distance $|\delta^{x_0}_a(s)-\delta^{z_0}_a(t)|_\Sigma$;
passing to the limit as $a\to \infty$, we get
\[|x_1-z_1|_\Sigma^2\ge|{x_0}-{z_0}|_\Sigma^2+(s-t)^2.\]

Now let swap ${x_0}$ with $x_1$ and ${z_0}$ with $z_1$,
...
for the pair $s=s_1+s_2$ and $t=t_1+t_2$.
Whence the claim follows for any pair $s$ and $t$.
...
In other words, each level set $L$ is either a closed or both sides infinite geodesic.

If $L$ is both sides infinite geodesic, then any point $x$ of $\Sigma$ can be uniquely described by two numbers $u=\bus_-(x)$ and the parameter $v$ of the intersection of the line thru $x$ in the direction of $\vec u$ with $L$.
By the claim $x\mapsto (u,v)$ produces an intrinsic isometry from $\Sigma$ to the Euclidean plane.

Analogously, if $L$ is is a closed geodesic, then claim implies existence  of an intrinsic isometry from $\Sigma$ to a circular cylinder.
\qeds
