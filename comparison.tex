...
\parit{Conclusions.}
Note that by \ref{prop:busemann}, it implies that $\bus_\pm\circ\lambda(t)=\mp t$ for any~$t$.
...
\parit{Step 3.}
Let us show that the distances between points on two lines in the direction of $\vec u$ behave the same way as the distances between parallel lines in Euclidean plane; a precise formulation is given in \ref{clm:parallel}.
...
the comparison (\ref{thm:comp:toponogov}) implies that
...
Since the triangle $[x_0\,\gamma(a)\,z_0]$ is fat (\ref{fat}) and if $a$ is large, then the two long sides of the triangle are close to $a$.
