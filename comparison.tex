...
It follows that, the surface $\Sigma$ can be subdivided into lines that run in the direction of $\vec u$.
...
\begin{thm}{Claim}
The distances between points on two lines in the direction of $\vec u$ behave the same way as the distances between parallel lines in Euclidean plane.

More precisely, let $\xi$ and $\zeta$ be two lines in $\Sigma$ that run in the direction of $\vec u$.
...
for the pair $s=s_1+s_2$ and $t=t_1+t_2$ and the claim follows.
...
Finally note that since $\bus_-$ is affine, the level set 
...
Analogously, if $L$ is is a closed geodesis, then claim implies existence  of an intrinsic isometry from $\Sigma$ to the circular cylinder.
\qeds
