...
Passing to a smaller neighborhood if necessary, we can assume that $U$ is simply connected.
Therefore this condition is not necessary to include in local version of part \ref{SHORT.thm:loc-comp:cba}.

\parit{Proof.}
Choose $p\in\Sigma$;
suppose that $\eps>0$ is provided by \ref{prop:exp}.
Take $U$ to be the $\tfrac\eps{4}$-neighborhood of $x$ in $\Sigma$.

Note that $y=\exp_x\vec v$ and $z=\exp_x\vec w$ for two vectors $\vec v,\vec w\in\T_x$ such that $|\vec v|, |\vec w|<\tfrac\eps2$, and
\begin{align*}
...
Since $|x-y|,|x-z|<\tfrac\eps{2}$, the triangle inequality implies that $\gamma$ lies in $\eps$-neighborhood of $x$.
In particular, there is a curve $\tilde \gamma$ in a $\eps$-neighborhood of zero in $\T_x$ that corresponds to $\gamma$; that is,  $\gamma=\exp_x\circ\tilde \gamma$.
