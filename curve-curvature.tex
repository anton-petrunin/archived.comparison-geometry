...
Let us define \emph{total signed curvature} for polygonal line in the plane as a sum of signed external angles;
the external angle has positive sign if the line turns right and negative sign if the line turns left; the signed external angle is undefined if a pair of adjacent edges overlap;
that is if at one vertex the polygonal line turns in the opposite direction.
...
Show that is is multiple of $2\cdot\pi$.
\end{thm}


\begin{thm}{Exercise}\label{ex:pm2pi}
Show that total signed curvature of any closed simple polygonal line in the plane is $\pm2\cdot\pi$.
\end{thm}
