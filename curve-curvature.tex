...
That the curve $\alpha$ can be extended to a smooth regular closed curve $\hat\alpha$ by an arc $\beta$ that starts from $\alpha(b)$ in the same direction as $\alpha$ turns and joins the segment $[\alpha(b),\alpha(a)]$ runs close to $\alpha(a)$ turns and joints $\alpha$ smoothly at $\alpha(a)$.
...
Note that since the curve $\alpha_u$ run back and forth along one line.
...
Since $n$ has to be even, $\tc\alpha_u$ may take values $2\cdot\pi, 4\cdot\pi,\dots$
...
The famous Fary--Milnor theorem states that states that the inequality is strict;
...
So the theorem above  

In the proof we will use the following fact: a coordinate function must have at least two local maximums and two local minimums on any nontrivial knot.
It is easy to prove assuming that we gave the definitions of nontrivial knot. 

\parit{Proof.}
It is sufficient to show that if $\alpha$ is a simple polygonal line that is a nontrival knot then its total curvature exceeds $2\cdot\pi$.

Note that for any unit vector $u$, the projection $\alpha_u$ has total curvature at least $4\cdot\pi$.
Indeed, $\alpha_u$ has even number of turns, each turns contributes $\pi$ to the total curvature. If the total curvature is less than $4\cdot\pi$ then the number of turns 2; that is $\alpha$ runs one way along a line segment and comes back.
\qeds
