...
Let $\gamma$ be a unit-speed smooth space curve and let $\gamma_{\lambda}$ be a scaled copy of $\gamma$ with factor $\lambda >0$;
that is, $\gamma_{\lambda}(t)=\lambda \cdot\gamma(t)$ for any $t$.
Show that 
\[  \kur(\lambda \cdot p )_{\gamma_{\lambda}}= \frac{1}{\lambda} \kur(p)_{\gamma}   \]
for any $p \in \gamma$.
...
\eqlbl{eq:tantrix}\] 
it is called \index{tangent indicatrix}\emph{tangent indicatrix} of $\gamma$.
...
Show that any smooth regular curve $\gamma\:\mathbb{I}\to\RR^3$ with curvature at most 1 can be approximated by a smooth curves with constant curvature 1.
...
If $\alpha'(b)$ and $\beta'(b)$ are defined, then the angle $\theta\z=\measuredangle(\alpha'(b),\beta'(b))$ is called \index{external angle}\emph{external angle} of $\gamma$ at time $b$.
...
It is possible to generalze most of the statments in this chapter to the (nonsmooth) curves of finite total curvature.
