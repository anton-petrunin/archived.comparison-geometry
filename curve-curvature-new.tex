...
The line thru $\gamma(s)$ in the direction of $\tau(s)$ is called \emph{tangent line} at $s$.
...
Let us show that $\tau$ cannot lie in a hemisphere.
...
Hint: Assume that $\gamma$ is unit-speed; show that $|\sigma'|\le \kur+\theta'$, where $\theta(s)=\angle(\gamma(s),\gamma'(s))$.
...
The same argument as in the proof of Fenchel's theorem, shows that the curves $\tau_1,\dots,\tau_n$ cannot lie in an open hemisphere.
...
$\tc\gamma$ is arbitrarily close to $\measuredangle(w,u)+\measuredangle(w,v)$, where $w=q-p$.
...
it implies the so-called \emph{F\'ary--Milnor theorem} --- the total curvature any knot exceeds $4\cdot \pi$.
...
Since the vectors $\gamma'_1(s),u_1,\gamma'_2(s),u_2$ are unit, it follows that 
...
Note that since $\gamma$ is smooth, the idicatrixes $\chi_n$ converge pointwise to $\tau$ --- the tangent indicatrix of $\gamma$.
