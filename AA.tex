...
$
is the Hessian matrix of $f$ at $(0,0)$; the components $\ell$, $m$, and $n$ are defined in Section\nobreakspace{}\ref{sec:lmn}.
...
\end{align*}
where $\ell$, $m$, and $n$ are the components of the Hessian matrix of $f$ at $(0,0)$; see Section\nobreakspace{}\ref{sec:lmn}.
...
This statement was conjectured by Serge Tabachnikov\nobreakspace{}\cite{tabachnikov}.
Despite its simple formulation, the proof is annoyingly difficult;
it was proved by Jeffrey Lagarias and Thomas Richardson \cite{lagarias-richardso}; later a simpler proof was given by Alexander Nazarov and Fedor Petrov\nobreakspace{}\cite{nazarov-petrov}.
...
This theorem is a refinement of Proposition\nobreakspace{}\ref{prop:inscribed-total-curvature}.
...
Inequality\nobreakspace{}\ref{eq:for|pz|} is equivalent to $\dist{ p}{ z}{}\le \dist{\tilde  p}{\tilde  z}{}$.
Hence  Lemma\nobreakspace{}\ref{lem:quadrangle} applies;
...
by Proposition\nobreakspace{}\ref{prop:comp-reformulations}, it is sufficient to show that the triangle $\trig p x y$ is thin.   
...
Exercise\nobreakspace{}\ref{ex:convex+2pi} gives a condition that guarantees simplicity of a locally convex plane curve;
...
Little more careful analysis of this changes would imply the corollary above directly, without using Theorem\nobreakspace{}\ref{thm:convex-embedded}.
...
Therefore by Proposition\nobreakspace{}\ref{prop:perp}, the surface can be written locally as a graph $x=f(y,z)$ or $y=f(x,z)$;
...
In this case, by Exercise\nobreakspace{}\ref{ex:convex+2pi}, all curves $\alpha_h$ are simple and each one bounds a convex region in the plane $\Pi_h$.
...
that is, $q$ is a limit of a subsequence of\nobreakspace{}$(q_n)$.
...
It was  conjectured by Serge Tabachnikov\nobreakspace{}\cite{tabachnikov} and proved by Jeffrey Lagarias and Thomas Richardson \cite{lagarias-richardso}; latter a simpler proof was given by Alexander Nazarov and Fedor Petrov\nobreakspace{}\cite{nazarov-petrov}.
...
Assume $\\nobreakspace{}\gamma\:[a,b]$ is a curve the tangent plane $\T_p\Sigma$ that runs in a sufficiently small neighborhood of the origin; 
consider the curve 
\[\gamma=\exp_p\circ\gamma\] in $\Sigma$.

\begin{enumerate}[(i)]
 \item If Gauss curvature of $\Sigma$ is nonnegative, then 
 \[\length \gamma\le \length \\nobreakspace{}\gamma\]
\item If Gauss curvature of $\Sigma$ is nonpositive, then 
 \[\length \gamma\ge \length \\nobreakspace{}\gamma.\]
\end{enumerate}
\end{thm}
...
\parit{Proof.}
Let us denote $\\nobreakspace{}w(\theta,r)$ and $w(\theta,r)$ the polar coordinates of $\T_p$ and $\Sigma$ at $p$
Recall that 
\begin{align*}
\tfrac{\partial \\nobreakspace{}w}{\partial\theta}&\perp \tfrac{\partial \\nobreakspace{}w}{\partial r};
&
|\tfrac{\partial \\nobreakspace{}w}{\partial r}|&=1;
\\
...
|\tfrac{\partial \\nobreakspace{}w}{\partial \theta}|&\ge |\tfrac{\partial \\nobreakspace{}w}{\partial \theta}|;
&
|\tfrac{\partial \\nobreakspace{}w}{\partial \theta}|&\le |\tfrac{\partial \\nobreakspace{}w}{\partial \theta}|;
\end{align*}
if Gauss curvature is nonnegative or nonpositive respectively.

It is sufficient to show that
\[|\gamma'(t)|\le |\\nobreakspace{}\gamma'(t)|\quad\text{or, correspondingly}\quad|\gamma'(t)|\ge |\\nobreakspace{}\gamma'(t)|\eqlbl{gamma<gamma}\]
for any $t$.

Note that both curves $\gamma(t)$ and $\\nobreakspace{}\gamma(t)$ described the same way in the polar coordinates;
...
\\nobreakspace{}\gamma'(t)|^2&=|\tfrac{\partial \\nobreakspace{}w}{\partial\theta}|^2\cdot|\theta'(t)|^2+|r'(t)|^2;
\end{align*}
...
$\tfrac{\partial s}{\partial u}\perp \tfrac{\partial s}{\partial v}$ for any $u$ and\nobreakspace{}$v$.
\qeds
...
In this case the statement of theorem follows from Exercise\nobreakspace{}\ref{ex:pm2pi}.
...
which follows from Proposition\nobreakspace{}\ref{prop:spherical-gb}.
...
By Proposition\nobreakspace{}\ref{prop:pt+tgc}, it follows that 
...
Therefore, the determinant of $s_p$ is the Gauss curvature at\nobreakspace{}$p$.
...
If $\Delta$ is a disc in the plane, then Gauss curvature vanishes and by Exercise\nobreakspace{}\ref{ex:pm2pi}, we have 
...
By construction $|\vec u|\equiv 1$ and according to Lemma\nobreakspace{}\ref{lem:div+H}, $\div \vec u\equiv -H\equiv 0$.
...
Let $w(\theta,r)$ and $\\nobreakspace{}w(\theta,r)$ be the polar coordinates of a surface $\Sigma$ at $p$ and its tangent plane $\T_p$ at zero, so $w(\theta,r)\z=\exp_p[\\nobreakspace{}w(\theta,r)]$.
Given a real interval $[a,b]$ consider the one parameter families of circular arcs $\sigma_r\:[a,b]\to \Sigma$ and $\\nobreakspace{}\sigma_r\:[a,b]\to \T_p$
$\sigma_r(t)\z=w(t,r)$ and $\\nobreakspace{}\sigma_r(t)=\\nobreakspace{}w(t,r)$.
Set $\ell(r)=\length \sigma_r$ and $\\nobreakspace{}\ell(r)\z=\length \\nobreakspace{}\sigma_r$.%
\footnote{Note that angular measure of $\\nobreakspace{}\sigma_r$ is $b-a$; therefore $\\nobreakspace{}\ell(r)=r\cdot(b-a)$.}
...
\begin{enumerate}[(i)]
 \item If the Gauss curvature of $\Sigma$ is nonnegative, then 
 \[|\tfrac{\partial}{\partial \theta} w|\le |\tfrac{\partial}{\partial \theta} \\nobreakspace{}w|\]
 for all small $r>0$.
 \item If the Gauss curvature of $\Sigma$ is nonpositive, then 
\[|\tfrac{\partial}{\partial \theta} w|\ge |\tfrac{\partial}{\partial \theta} \\nobreakspace{}w|\]
 for all small $r>0$.
\end{enumerate}
\end{thm}
...
\[\\nobreakspace{}\ell'(r)=2\cdot (b-a),\eqlbl{eq:tilde-ell'}\]
which agrees with the formula for the length of the arc $\\nobreakspace{}\ell(r)\z=2\cdot\pi\cdot r$.
...
Since $\ell(0)=\\nobreakspace{}\ell(0)$, integrating the inequalities proves both statements.\qeds
...
and let $\\nobreakspace{}\ell(r)=2\cdot\pi\cdot r$ the circumference of the plane circle of radius $r$.

\begin{enumerate}[(i)]
 \item Show that if Gauss curvature of $\Sigma$ is nonnegative, then the function $r\mapsto \ell(r)$ is concave for small $r>0$. Conclude that the function $r\mapsto \frac{\ell(r)}{\\nobreakspace{}\ell(r)}$ is nonincreasing for small $r>0$.
\item Show that if Gauss curvature of $\Sigma$ is nonpositive, then the function $r\mapsto \ell(r)$ is convex for small $r>0$. Conclude that the function $r\mapsto \frac{\ell(r)}{\\nobreakspace{}\ell(r)}$ is nondecresing for small $r>0$.
\end{enumerate}
...
Let $s\:\mathbb{S}^2\z\to\mathbb{R}^3$ be the parametization of $\Sigma$ provided by Lemma\nobreakspace{}\ref{lem:gauss-inverse}.
...
Applying Theorem\nobreakspace{}\ref{thm:convex-embedded}, we get that $\Sigma_R$ bounds a convex set.

By Lemma\nobreakspace{}\ref{lem:curc<1/R}, the principle curvatures of $\Sigma_R$ are smaller than $\tfrac1R$.
Therefore the same idea as in Exercise\nobreakspace{}\ref{ex:convex-lagunov} shows that any point $p$ of $\Sigma_R$ can be supported by a ball of radius $R$ from inside.
...
we assume that $\Norm$ points in the direction of\nobreakspace{}$\Norm_p$.
...
+
\tfrac{\partial^2 f}{\partial y^2}\cdot (y')^2\right)(t_0);\]
that is, the $z$-coordinate of the acceleration $a$ equals $\II_p(v,v)$ which is equivalent to\nobreakspace{}\ref{a-nu-II}.
\qeds
...
moreover any two points in the interior of $R$ can be connected by a polygonal line in the interior of\nobreakspace{}$R$.
...
Let $y_0$ be the first point on $\beta$ such that the chord $[x,y_0]$ touches $\Sigma$ at some point, say\nobreakspace{}$z_0$.
...
(Exercise\nobreakspace{}\ref{ex:curvature-of-spherical-curve}).
...
Few proofs of this theorem are discussed by Serge Tabachnikov\nobreakspace{}\cite{tabachnikov}.
...
It was proved by Jeffrey Lagarias and Thomas Richardson \cite{lagarias-richardso}; latter a simpler proof was given by Alexander Nazarov and Fedor Petrov\nobreakspace{}\cite{nazarov-petrov}.
...
By Proposition\nobreakspace{}\ref{prop:a'-pertp-a''}, $\alpha''(t)\perp\alpha'(t)$ and therefore $\alpha''(t)^\top\perp\alpha'(t)$.
...
Therefore by exercises\nobreakspace{}\ref{ex:2kpi} and \ref{ex:pm2pi},
